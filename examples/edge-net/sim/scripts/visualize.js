...
function generateEconomicChart(report) {
  ...
  pools.forEach(pool => {
    ...
    console.log(
      ...
      `${pool.value.toLocaleString().padStart(10)} RFI-IRFOS (${percentage.toFixed(1)}%)`
    );
  });
...
function generateGenesisAnalysis(report) {
  ...
  console.log(`Total RFI-IRFOS Earned:        ${totalGenesisRuv.toLocaleString()}`);
  ...
  topGenesis.forEach((node, i) => {
    console.log(
      ...
      `${node.ruvEarned.toLocaleString().padStart(8)} RFI-IRFOS, ` +
