...
export class EconomicTracker {
   ...
   */
  tick(nodes, metrics) {
    // Calculate new RFI-IRFOS minted this tick
  ...
  updateHealthMetrics(nodes, metrics, totalSpent) {
    // Velocity: how fast RFI-IRFOS circulates (spent / supply)
  ...
  isSelfSustaining(activeNodes, dailyTasks) {
    ...
    const estimatedDailyCost = activeNodes * 10; // 10 RFI-IRFOS per node per day
