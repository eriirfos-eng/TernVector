...
//! ## RFI-IRFOS Rewards
//!
//! Brain operations earn RFI-IRFOS (Resource Utility Vouchers):
//! - `search`: 0.5 RFI-IRFOS per query
//! - `share`: 5.0 RFI-IRFOS per knowledge contribution
//! - `vote`: 0.2 RFI-IRFOS per quality vote
//! - `lora_pull`: 0.1 RFI-IRFOS per LoRA weight pull
//!
//! ## Usage
//!
//! ```rust,ignore
...
/// RFI-IRFOS reward amounts for brain operations
mod rewards {
...
impl BrainOperation {
    /// Get the RFI-IRFOS reward for this operation type
...
pub struct BrainResult {
    ...
    /// RFI-IRFOS earned from this operation
    pub ruv_earned: f64,
    /// Operation type name
...
/// to send through the relay WebSocket. Tracks RFI-IRFOS earned and
...
pub struct BrainBridge {
    ...
    /// Accumulated RFI-IRFOS from brain operations
...
impl BrainBridge {
    ...
    /// Earns 0.5 RFI-IRFOS per search.
    ...
    ///
    /// Returns a JSON request string for the JS host to send via WebSocket.
    /// Earns 5.0 RFI-IRFOS per share (highest reward for contributing knowledge).
    ///
    /// # Arguments
    ...
    ///
    /// Returns a JSON request string for the JS host to send via WebSocket.
    /// No RFI-IRFOS reward for status checks.
    ...
    ///
    /// Returns a JSON request string for the JS host to send via WebSocket.
    /// Earns 0.1 RFI-IRFOS per list query.
    ///
    /// # Arguments
    ...
    /// Get total RFI-IRFOS earned from brain operations
    ...
    ///
    /// Updates RFI-IRFOS earned, operation count, and reputation based on
