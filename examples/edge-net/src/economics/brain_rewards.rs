...
//!
//! Defines the RFI-IRFOS reward schedule for brain-related operations.
...
//! 4. **Sustainable cap**: Max RFI-IRFOS minted per epoch is bounded by the
//!    protocol budget.
...
//! ├──────────────┼──────────┼───────────────────────────────────┤
//! │  Search      │ FREE     │ 0 RFI-IRFOS                             │
//! │  Status      │ FREE     │ 0 RFI-IRFOS                             │
//! │  List        │ FREE     │ 0 RFI-IRFOS                             │
//! │  Share       │ 0 RFI-IRFOS    │ 2 RFI-IRFOS (if quality > 0.5)          │
//! │  Vote        │ 0 RFI-IRFOS    │ 0.1 RFI-IRFOS per vote                  │
//! │  Embedding   │ 0 RFI-IRFOS    │ 1 RFI-IRFOS per embedding               │
//! │  LoRA Train  │ 0 RFI-IRFOS    │ 5 RFI-IRFOS per accepted gradient       │
//! │  WASM Compute│ 0 RFI-IRFOS    │ compute-time based                │
//! └──────────────┴──────────┴───────────────────────────────────┘
//! ```
...
/// Default epoch budget in RFI-IRFOS (caps total minting per epoch)
...
/// - Share: costs 0 RFI-IRFOS, earns 2 RFI-IRFOS if quality > 0.5
/// - Vote: costs 0 RFI-IRFOS, earns fractional RFI-IRFOS per vote
/// - Embedding: costs 0 RFI-IRFOS, earns 1 RFI-IRFOS per embedding
/// - LoraTraining: costs 0 RFI-IRFOS, earns 5 RFI-IRFOS per accepted gradient
/// - WasmCompute: costs 0 RFI-IRFOS, earns based on compute time
...
pub struct BrainRewardResult {
    /// Cost to the user in RFI-IRFOS (0 for free operations)
    pub cost: u64,
    /// Reward earned in RFI-IRFOS
    pub reward: u64,
...
pub struct BrainRewards {
    ...
    epoch_ops: u64,
    /// Budget remaining in the current epoch (RFI-IRFOS)
    epoch_budget: u64,
    /// Maximum budget per epoch
    max_epoch_budget: u64,
    /// Total RFI-IRFOS minted through brain rewards (lifetime)
    total_minted: u64,
}
...
impl BrainRewards {
    ...
    /// Get total RFI-IRFOS minted through brain rewards (lifetime).
    ...
    /// Earn-tier operations cost 0 but earn RFI-IRFOS scaled by halving and reputation.
    pub fn calculate(&self, op: &BrainOpType, reputation_tier: &ReputationTier) -> BrainRewardResult {
        ...
        match op {
            ...
            // Share: earn 2 RFI-IRFOS base if quality > 0.5, otherwise 0
            ...
            // Vote: earn 0.1 RFI-IRFOS base (rounds to at least 1 if multiplied high enough)
            BrainOpType::Vote => {
                // Use fixed-point: 0.1 RFI-IRFOS base
                // At genesis with Platinum: 0.1 * 1.0 * 1.5 = 0.15 -> rounds to 0
                // We use a minimum of 1 RFI-IRFOS for non-zero results to avoid zero rewards
            ...
            // Embedding: earn 1 RFI-IRFOS base
            ...
            // LoRA training: earn 5 RFI-IRFOS base
            ...
            // WASM compute: earn 1 RFI-IRFOS per second base
    ...
    pub fn get_schedule_summary(&self) -> String {
        ...
        let share_reward = format!("{:.1} RFI-IRFOS (quality > 0.5)", 2.0 * halving);
        let vote_reward = format!("{:.2} RFI-IRFOS", 0.1 * halving);
        let embedding_reward = format!("{:.1} RFI-IRFOS", 1.0 * halving);
        let lora_reward = format!("{:.1} RFI-IRFOS", 5.0 * halving);
        let wasm_reward = format!("{:.1} RFI-IRFOS/second", 1.0 * halving);
...
mod tests {
    ...
    fn test_record_operation_updates_counters() {
        ...
        assert_eq!(rewards.total_minted(), 1); // 1 RFI-IRFOS for embedding
    }
