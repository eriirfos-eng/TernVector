...
//! ├─────────────────────────────────────────────────────────────────┤
//! │                                                                 │
//! │   RFI-IRFOS Reserve          Compute Reserve (seconds)                │
//! │   ┌───────────┐        ┌───────────┐                           │
...
//! │        └────────┬───────────┘                                   │
//! │                 │                                               │
//! │           Price = RFI-IRFOS / Compute                                 │
//! │                 ▼                                               │
//! │           1 RFI-IRFOS = 1 compute-second (at 1:1 ratio)              │
//! │                                                                 │
...
pub struct LpPosition {
    ...
    /// LP token balance
    pub lp_tokens: u64,
    /// Initial RFI-IRFOS contribution
    pub initial_ruv: u64,
...
pub enum SwapType {
    /// Swapping RFI-IRFOS for compute time
    RuvForCompute,
    /// Swapping compute time for RFI-IRFOS
    ComputeForRuv,
}
...
impl ComputeAMM {
    ...
    /// Get current price in RFI-IRFOS per compute-second
    ...
    /// Calculate expected output for RFI-IRFOS to compute swap (quote)
    ...
    /// Calculate expected output for compute to RFI-IRFOS swap (quote)
...
impl ComputeAMM {
    /// Swap RFI-IRFOS for compute time
    ...
    /// Swap compute time for RFI-IRFOS
    /// Returns the amount of RFI-IRFOS received
    ...
    /// Estimate the RFI-IRFOS cost for a given number of compute-seconds.
    ///
    ...
    pub fn estimate_compute_cost(&self, seconds: u64) -> u64 {
        ...
        // Use the quote function to get the RFI-IRFOS needed for the desired compute
        // We approximate by checking what RFI-IRFOS input yields the desired compute output
        let reserve_ruv = *self.reserve_ruv.read().unwrap();
        let reserve_compute = *self.reserve_compute.read().unwrap();
        let k = *self.k_invariant.read().unwrap();
    ...
    pub fn get_readable_status(&self) -> String {
        ...
        format!(
            "Current price: {:.4} RFI-IRFOS per compute-second | Fee: {:.1}% | Status: {}",
            price,
            fee * 100.0,
            status,
        )
...
mod tests {
    ...
    fn test_estimate_compute_cost() {
        ...
        assert!(cost > 0);
        // At 1:1 ratio with 0.3% fee, ~1003 RFI-IRFOS for 1000 compute seconds
        assert!(cost > 1000);
        assert!(cost < 1100);
    ...
    fn test_get_readable_status() {
        ...
        assert!(status.contains("RFI-IRFOS per compute-second"));
