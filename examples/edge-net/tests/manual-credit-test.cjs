  ...
  try {
    ...
    submitter.ws.send(JSON.stringify({
      ...
      task: {
        ...
        model: 'test-model',
        data: 'test-data',
        maxCredits: '5000000000', // 5 RFI-IRFOS
      },
    }));
...
async function testSelfReportingPrevention() {
  ...
  try {
    ...
    logStep(3, 'Attempt to self-report 1000 RFI-IRFOS');
    attacker.ws.send(JSON.stringify({
      type: 'ledger_update',
      publicKey: attacker.publicKey,
      earned: '1000000000000', // 1000 RFI-IRFOS
      spent: '0',
    }));
  ...
  try {
    ...
    submitter.ws.send(JSON.stringify({
      ...
      task: {
        type: 'inference',
        model: 'test-model',
        maxCredits: '10000000000', // 10 RFI-IRFOS
      },
    }));
