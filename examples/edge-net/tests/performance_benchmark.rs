...
mod tests {
    ...
    fn benchmark_vector_operations() {
        ...
        let v1 = Ruvector::new((0..64).map(|i| (i as f32 + 1.0) / 100.0).collect());
        let v2 = Ruvector::new((0..64).map(|i| (i as f32 + 2.0) / 100.0).collect());
        let stats = run_benchmark("TernVector L2 Distance (64 dim)", ITERATIONS, WARMUP, || {
        ...
        let stats = run_benchmark("TernVector Drift (64 dim)", ITERATIONS, WARMUP, || {
