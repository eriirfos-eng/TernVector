...
async function main() {
  ...
  for (const { identity, ws } of connections) {
    ...
    console.log(`  ${identity.name}: ${balance ? balance.earned.toFixed(4) : 'N/A'} RFI-IRFOS`);
  }
  ...
  const contributions = [
    { seconds: 30, cpu: 50 },  // ~0.705 RFI-IRFOS
    { seconds: 30, cpu: 40 },  // ~0.564 RFI-IRFOS
    { seconds: 30, cpu: 30 },  // ~0.423 RFI-IRFOS
  ];
  ...
  for (let i = 0; i < contributorConnections.length; i++) {
    ...
    if (response.type === 'contribution_credit_success') {
      ...
      console.log(`  ${c.green}✓${c.reset} ${identity.name}: ${credited.toFixed(4)} RFI-IRFOS credited`);
  ...
  for (const { identity, ws } of connections) {
    ...
    console.log(`  ${identity.name}: ${balance?.earned?.toFixed(4) || 'N/A'} RFI-IRFOS (${changeStr})`);
  }
