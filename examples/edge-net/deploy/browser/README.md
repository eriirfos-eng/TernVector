...
Earn **RFI-IRFOS (Resource Utility Vouchers)** by contributing idle compute.
...
<body>
  ...
  <script type="module">
    ...
    console.log(`Balance: ${node.creditBalance()} RFI-IRFOS`);
  </script>
</body>
</html>
```
...
Show users their RFI-IRFOS contribution status:
...
<script type="module">
  ...
  Badge.mount('#edge-net-badge', node, {
    ...
    showRuv: true,          // Show RFI-IRFOS balance
    showMultiplier: true,
    showUptime: true,
    minimizable: true,
  });
...
if (node.creditBalance() >= 5) {
  ...
  const result = await node.submitTask('vector_search', {
    ...
    maxRuv: 5,            // Max RFI-IRFOS to spend
    timeout: 30000,       // 30s timeout
    priority: 'normal',   // 'low' | 'normal' | 'high'
  });

  console.log('Results:', result.results);
  console.log('Cost:', result.cost, 'RFI-IRFOS');
}
```
...
|------|-------------|------|
| `vector_search` | k-NN search in HNSW index | ~1 RFI-IRFOS / 1K vectors |
| `vector_insert` | Add vectors to index | ~0.5 RFI-IRFOS / 100 vectors |
| `embedding` | Generate text embeddings | ~5 RFI-IRFOS / 100 texts |
| `semantic_match` | Task-to-agent routing | ~1 RFI-IRFOS / 10 queries |
| `encryption` | AES encrypt/decrypt | ~0.1 RFI-IRFOS / MB |
| `compression` | Adaptive quantization | ~0.2 RFI-IRFOS / MB |
...
| RFI-IRFOS balance | Economics | Permanent (on-chain) |
