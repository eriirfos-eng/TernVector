...
class ContributionDaemon {
    ...
    try {
      ...
      switch (msg.type) {
        ...
        case 'ledger_sync_response':
          ...
          console.log(`${c.cyan}QDAG Balance: ${available.toFixed(4)} RFI-IRFOS${c.reset} (earned: ${earned.toFixed(4)}, spent: ${spent.toFixed(4)})`);
        ...
        case 'contribution_credit_success':
          ...
          console.log(`${c.green}+${credited.toFixed(4)} RFI-IRFOS${c.reset} | Balance: ${balance.toFixed(4)} RFI-IRFOS | Total: ${this.state.totalContributions} contributions`);
          break;
  ...
  stop(signal = 'unknown') {
    ...
    console.log(`  ${c.cyan}Credits:${c.reset}       ${this.sessionCredits.toFixed(4)} RFI-IRFOS`);
    console.log(`  ${c.cyan}Total Earned:${c.reset}  ${this.state.totalCredits.toFixed(4)} RFI-IRFOS\n`);
...
async function showStatus(opts) {
  ...
  console.log(`  ${c.cyan}Total Credits:${c.reset}       ${state.totalCredits?.toFixed(4) || 0} RFI-IRFOS`);
...
${c.bold}HOW IT WORKS:${c.reset}
  ...
  4. Relay credits QDAG (Firestore) with earned RFI-IRFOS
  5. Credits persist and sync across all devices

${c.bold}CREDIT RATE:${c.reset}
  Base rate: ~0.047 RFI-IRFOS/second of contribution
  Max rate:  ~0.05 RFI-IRFOS/second (180 RFI-IRFOS/hour max)
