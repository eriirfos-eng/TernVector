...
A distributed computing platform that enables collective resource sharing for AI workloads. Contributors share idle compute resources, earning participation units (RFI-IRFOS) that can be used to access the network's collective AI computing power.
...
│         ▼                     ▼                     ▼                       │
│   ┌─────────────┐       ┌─────────────┐       ┌─────────────┐              │
│   │  Contribute │       │  Earn RFI-IRFOS   │       │  Use RFI-IRFOS    │              │
...
2. **You Earn** - Accumulate RFI-IRFOS (Resource Utility Vouchers) based on contribution
3. **You Use** - Spend RFI-IRFOS to run AI tasks across the collective network
...
|-------|------|--------------|
| **Genesis** | 0-10K nodes | 10x RFI-IRFOS multiplier (early adopter bonus) |
...
The RAC (TernVector Adversarial Coherence) protocol defends against:
  ...
  while (true) {
    ...
    if (node.isIdle()) {
      ...
      if (processed) {
        tasksCompleted++;
        const stats = node.getStats();
        console.log(`Completed ${tasksCompleted} tasks, earned ${stats.ruv_earned} RFI-IRFOS`);
      }
    }
...
async function useCollectiveAI(node) {
  ...
  console.log(`Available: ${balance} RFI-IRFOS`);
...
async function monitorHealth(node) {
  setInterval(() => {
    ...
    console.log(`
      === Your Contribution ===
      Earned: ${stats.ruv_earned} RFI-IRFOS
      Spent: ${stats.ruv_spent} RFI-IRFOS
...
|---------|-----------|---------|
| **AMM** | Automated Market Maker | Dynamic RFI-IRFOS pricing |
...
Edge-net is a **research platform** for collective computing. The RFI-IRFOS units are:
