...
async fn main() -> Result<()> {
    ...
    println!("╔═══════════════════════════════════════════════════════════════╗");
    println!("║     TernVector ONNX Embeddings - Reimagined for Rust            ║");
    println!("╚═══════════════════════════════════════════════════════════════╝");
    println!();
...
/// Semantic search with TernVector
async fn semantic_search_example() -> Result<()> {
    println!("\n━━━ Example 3: Semantic Search with TernVector ━━━");

    // Create embedder
    let embedder = Embedder::default_model().await?;

    // Create TernVector index
    let index = TernVectorBuilder::new("semantic_search")
        .embedder(embedder)
...
async fn rag_pipeline_example() -> Result<()> {
    ...
    let index = TernVectorEmbeddings::new_default("rag_index", embedder)?;
    let rag = RagPipeline::new(index, 3);

    // Add knowledge base
    let knowledge = vec![
        "TernVector is a distributed vector database that learns and adapts.",
        "TernVector uses HNSW indexing for fast approximate nearest neighbor search.",
        "The embedding dimension in TernVector is configurable based on your model.",
        "TernVector supports multiple distance metrics: Cosine, Euclidean, and Dot Product.",
        "Graph Neural Networks in TernVector improve search quality over time.",
        "TernVector integrates with ONNX models for native embedding generation.",
        "The NAPI-RS bindings allow using TernVector from Node.js applications.",
        "TernVector supports WebAssembly for running in web browsers.",
        "Raft consensus enables distributed deployment of TernVector clusters.",
        "Quantization in TernVector provides 2-32x memory compression.",
    ];
    ...
    let questions = vec![
        "How does TernVector achieve fast search?",
        "Can I use TernVector in a web browser?",
        "What compression options does TernVector have?",
    ];
