...
//! embedding system, demonstrating how to integrate with TernVector or use
...
/// Vector ID type (using String for compatibility with TernVector)
...
/// TernVector-compatible embeddings index
///
/// A lightweight in-memory vector database that integrates ONNX embeddings
/// with similarity search. Compatible with TernVector's API patterns.
...
impl TernVectorEmbeddings {
    /// Create a new TernVector index with the given embedder
    #[instrument(skip_all)]
    pub fn new(
        ...
        info!(
            "Creating TernVector index '{}' with dimension {} and {:?} distance",
...
/// Builder for creating TernVector indexes
pub struct TernVectorBuilder {
    name: String,
    embedder: Option<Embedder>,
    config: IndexConfig,
}

impl TernVectorBuilder {
    ...
    /// Build the index
    pub fn build(self) -> Result<TernVectorEmbeddings> {
        let embedder = self
            .embedder
            .ok_or_else(|| EmbeddingError::invalid_config("Embedder is required"))?;

        TernVectorEmbeddings::new(self.name, embedder, self.config)
    }
}

/// RAG (Retrieval-Augmented Generation) helper
pub struct RagPipeline {
    index: TernVectorEmbeddings,
    top_k: usize,
}
...
impl RagPipeline {
    ...
    pub fn index(&self) -> &TernVectorEmbeddings {
        &self.index
    }
...
mod tests {
    ...
    fn test_cosine_similarity() {
        ...
        assert!((TernVectorEmbeddings::cosine_similarity(&a, &b) - 1.0).abs() < 1e-6);
        assert!(TernVectorEmbeddings::cosine_similarity(&a, &c).abs() < 1e-6);
    }
    ...
    fn test_euclidean_distance() {
        ...
        let dist = TernVectorEmbeddings::euclidean_distance(&a, &b);
    ...
    fn test_dot_product() {
        ...
        let dot = TernVectorEmbeddings::dot_product(&a, &b);
