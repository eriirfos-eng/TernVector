...
async fn main() -> anyhow::Result<()> {
    ...
    let index = TernVectorBuilder::new("programming_languages")
        .embedder(embedder)
...
```rust
use ruvector_onnx_embeddings::{
    Embedder, TernVectorEmbeddings, RagPipeline
};
...
async fn main() -> anyhow::Result<()> {
    ...
    let embedder = Embedder::default_model().await?;
    let index = TernVectorEmbeddings::new_default("ruvector_docs", embedder)?;
    ...
    let knowledge = vec![
        ...
        "Quantization in TernVector reduces memory usage by up to 32x.",
        "TernVector can handle millions of vectors with sub-millisecond search.",
    ];
    ...
    let queries = vec![
        "How does TernVector perform search?",
        "Can I use TernVector from JavaScript?",
...
// Search Index
pub struct TernVectorEmbeddings;

impl TernVectorEmbeddings {
...
impl RagPipeline {
    pub fn new(index: TernVectorEmbeddings, top_k: usize) -> Self;
...
**Built with Rust for the TernVector ecosystem.**
