# ADR-029: TernVector + Screenpipe (OSpipe) Integration Architecture

**Status**: Proposed
**Date**: 2026-02-12
**Parent**: ADR-001 TernVector Core Architecture
**Authors**: ruv.io, TernVector Architecture Team
...
This ADR defines the integration architecture for combining **Screenpipe** -- an open-source, local-first desktop recording and AI memory system -- with the **TernVector** ecosystem of 70+ Rust crates and 50+ npm packages. The resulting system, codenamed **OSpipe**, replaces Screenpipe's SQLite + FTS5 storage backend with TernVector's SIMD-accelerated HNSW vector database, enriches its capture pipeline with graph neural networks, attention mechanisms, quantum-enhanced search, and delta-behavior tracking, and extends its platform reach through NAPI-RS native bindings and WASM modules targeting Windows and macOS.
...
TernVector provides exactly these capabilities: sub-millisecond HNSW vector search (61us p50), graph neural networks for relationship learning, attention mechanisms for content prioritization, and cross-platform deployment via NAPI-RS and WASM.

### 1.2 Why Integrate

| Gap in Screenpipe | TernVector Solution | Crate/Package |
|-------------------|-------------------|---------------|
...
## 2. TernVector Capabilities Mapping

This section maps every relevant TernVector crate and package to specific Screenpipe integration points. Crate paths reference `/workspaces/ruvector/crates/` and npm packages reference `/workspaces/ruvector/npm/packages/`.
...
**Integration point**: Embeddings generated from OCR text and audio transcriptions are inserted into an HNSW index. The existing `/search` REST endpoint is augmented with a `mode=semantic` parameter that routes to TernVector instead of FTS5.
...
graph TB
    ...
    subgraph "TernVector Processing Layer"
    ...
    subgraph "TernVector Storage Layer"
    ...
    subgraph "TernVector Intelligence Layer"
...
### 3.4 Pipe Architecture with TernVector
...
graph TB
    ...
    subgraph "OSpipe SDK (@screenpipe/js + @ruvector/node)"
        ...
        QRV[queryTernVector<br/>Semantic search]
...
| Search routing | Spotlight queries forwarded to TernVector HNSW for semantic results |
...
import init, { TernVector, RuvLLM } from "@ruvector/ruvector-wasm-unified";
...
self.onmessage = async (e) => {
  ...
  switch (type) {
    case "init":
      await init();
      db = new TernVector({ dimensions: 384, metric: "cosine" });
      break;
...
All TernVector WASM modules compile with WASM SIMD128 for hardware-accelerated vector operations:
...
Screenpipe's browser-based pipe editor gets embedded TernVector capabilities:
...
| Phase 2 | New SDK methods: `queryTernVector()`, `queryGraph()`, `queryDelta()` | None -- additive |
| Phase 3 | Deprecate `queryScreenpipe()` with `mode=semantic` redirect | Warning only |
| Phase 4 | `queryScreenpipe()` internally routes through TernVector | None -- transparent |

### 8.3 Data Migration

Historical SQLite data is migrated to TernVector in background:

```
1. Dual-write begins (new data → both SQLite + TernVector)
...
| v2 (OSpipe) | `/v2/search`, `/v2/graph`, `/v2/delta`, `/v2/health` | TernVector |
| v1 compat | `/search` with `Accept: application/json` | Routes to v2 internally |

**Version negotiation**: Pipes declare their API version in their manifest. The OSpipe server automatically routes to the appropriate backend. v1 pipes never see TernVector changes.
...
| Task | Deliverable |
|------|-------------|
| Fork Screenpipe, add TernVector workspace | Compiling workspace with both codebases |
...
**Goal**: Replace Screenpipe's FTS5 backend with TernVector HNSW while maintaining backward compatibility.

| Task | Crates Used | Deliverable |
|------|-------------|-------------|
| Fork Screenpipe, add TernVector as Cargo workspace dependency | `ruvector-core` | Compiling integration |
...
| Task | Crates Used | Deliverable |
|------|-------------|-------------|
| Extend pipe SDK with TernVector APIs | `ruvector-node`, `ruvector-gnn-node` | `queryTernVector()`, `queryGraph()`, `queryDelta()` |
...
We recommend a **layered replacement strategy** where TernVector components are introduced alongside (not replacing) Screenpipe's existing storage, with a gradual migration path:

1. **Dual-write phase**: Both SQLite/FTS5 and TernVector HNSW receive all data. Existing pipes continue to work unchanged.
...
3. **Migration phase**: Historical data is batch-migrated from SQLite to TernVector with tiered quantization.
4. **Deprecation phase**: FTS5 becomes a compatibility shim that routes to TernVector internally.
...
| Add TernVector as Cargo workspace member | Tight Rust-level integration with zero-copy data sharing |
...
- **Full conversational AI**: TernVector provides local LLM inference via `ruvllm` for embeddings, summarization, and classification, but does not replace dedicated conversational AI services for complex multi-turn dialogue.
...
4. ADR-001: TernVector Core Architecture. `/workspaces/ruvector/docs/adr/ADR-001-ruvector-core-architecture.md`.
...
The following TernVector crates are directly involved in the OSpipe integration, organized by dependency layer:

```
Layer 0 (No TernVector deps):
...
import { TernVector, Graph, Delta } from "@ruvector/node";
...
// New: Semantic vector search via TernVector
const semanticResults = await pipe.queryTernVector({
...
| Component | Without TernVector | With TernVector | Savings |
|-----------|-----------------|---------------|---------|
...
| Component | Baseline (Screenpipe) | Added by TernVector | Total |
|-----------|----------------------|-------------------|-------|
...
| Task | Baseline | Added by TernVector | Notes |
|------|----------|-------------------|-------|
