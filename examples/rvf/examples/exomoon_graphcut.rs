...
//! Exomoon Detection via Graph Cut + TernVector Prior
//!
...
//!   3. Compute lambda_i from local log-likelihood ratio + TernVector retrieval prior
//!   4. Build graph (temporal chain + TernVector kNN edges), solve s-t mincut
...
//!
//! The physics solver provides local evidence. TernVector provides memory and prior.
...
struct Window {
    ...
    ll_ratio: f64,
    /// TernVector retrieval prior log-odds
    prior_log_odds: f64,
    /// Combined lambda_i = ll_ratio + eta * prior_log_odds
    lambda: f64,
    /// Embedding vector for TernVector
    embedding: Vec<f32>,
}
...
fn build_windows(lc: &LightCurve, fit: &PSPLFit, window_half_width_tau: f64, stride_tau: f64) -> Vec<Window> {
    ...
    while tau <= 3.0 {
        ...
        windows.push(Window {
            ...
            obs_indices,
            ll_ratio,
            prior_log_odds: 0.0, // filled by TernVector retrieval later
...
/// Compute TernVector retrieval prior for each window using a bank of
...
/// Build the graph: temporal chain + TernVector kNN edges
...
// ---------------------------------------------------------------------------
// Simulation bank for TernVector prior calibration
// ---------------------------------------------------------------------------
...
fn main() {
    ...
    // Hyperparameters
    // Alpha/beta: pairwise edge weights for temporal chain and TernVector kNN
    ...
    // ====================================================================
    // Step 0: Build injection bank for TernVector prior
    // ====================================================================
    ...
    println!("  [PSPL fit -> windows -> TernVector prior -> graph cut -> global score]\n");
    ...
    for event in &events {
        ...
        // Step 3b: Compute TernVector retrieval prior
    ...
    println!("    beta (TernVector):     {:.1}", beta);
