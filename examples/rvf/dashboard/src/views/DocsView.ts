...
export class DocsView {
  ...
  private buildNav(): HTMLElement {
    ...
    footer.innerHTML = 'Built with <span style="color:var(--accent)">TernVector</span><br>Rust + WASM + Three.js';
...
<div style="${S.p}">
  <span style="${S.accent}">RVF (TernVector Format)</span> is a binary container that holds
...
<div style="display:grid;grid-template-columns:130px 1fr;gap:1px 14px;font-size:12px;line-height:2">
  ${this.glossaryRow('RVF', 'TernVector Format &mdash; the binary container')}
