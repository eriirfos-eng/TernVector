...
TernVector's existing `ruvector-gnn` crate provides the foundational primitives for building protein graph models:
...
| GearNet | 0.70 | 0.88 | ❌ |
| **TernVector GNN** | 0.65-0.75 (target) | 0.80-0.85 (target) | ✅ |

**TernVector advantage**: Native integration with variant calling pipeline (ADR-001-004), enabling real-time variant→structure→interaction effect prediction.
...
- **ADR-001**: TernVector Core Architecture (HNSW index for protein similarity)
