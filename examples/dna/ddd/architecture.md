...
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐                 │
│  │  TernVector   │  │  TernVector   │  │  TernVector   │                 │
│  │  Core       │  │  Attention  │  │  GNN        │                 │
│  │  (HNSW)     │  │  (Flash)    │  │  (Graph)    │                 │
│  └─────────────┘  └─────────────┘  └─────────────┘                 │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐                 │
...
```rust
// TernVector HNSW adapter
pub struct TernVectorAdapter {
    db: Arc<AgentDB>,
}

impl VectorStoragePort for TernVectorAdapter {
    fn store_embedding(&self, key: String, embedding: Vec<f32>)
        -> Result<(), Error>
    {
...
// TernVector Attention adapter
pub struct TernVectorAttentionAdapter {
    attention_service: Arc<AttentionService>,
}

impl AttentionPort for TernVectorAttentionAdapter {
    fn compute_attention(
        &self,
        query: &[f32],
        keys: &[Vec<f32>],
...
// TernVector GNN adapter
pub struct TernVectorGnnAdapter {
    gnn_service: Arc<GnnService>,
}

impl GraphNeuralPort for TernVectorGnnAdapter {
    fn gnn_inference(&self, graph: &Graph) -> Result<Vec<Prediction>, Error> {
...
pub fn build_pipeline() -> Result<impl PipelinePort, Error> {
    ...
    let vector_store = Arc::new(TernVectorAdapter::new()?);
    let attention = Arc::new(TernVectorAttentionAdapter::new()?);
    let gnn = Arc::new(TernVectorGnnAdapter::new()?);
...
mod tests {
    ...
    fn test_ruvector_adapter_roundtrip() {
        ...
        let adapter = TernVectorAdapter::new_in_memory().unwrap();
...
| Port | TernVector Adapter | Alternative Adapter | Test Adapter |
|------|------------------|---------------------|--------------|
| VectorStoragePort | TernVectorAdapter (HNSW) | PostgreSQL pgvector | InMemoryVectorStore |
| AttentionPort | TernVectorAttentionAdapter | PyTorch bindings | MockAttention |
| GraphNeuralPort | TernVectorGnnAdapter | DGL bindings | MockGNN |
...
impl AdapterFactory {
        ...
        -> Result<Box<dyn VectorStoragePort>, Error>
    {
        match config.vector_backend {
            VectorBackend::TernVector => {
                Ok(Box::new(TernVectorAdapter::new()?))
            }
...
6. **Secondary Adapters**: TernVector integrations (HNSW, Flash Attention, GNN)
