...
7sense is a bioacoustics platform built on TernVector that enables researchers to upload audio recordings, extract call segments, generate embeddings via Perch 2.0, and query a graph database of acoustic relationships. The platform requires a comprehensive API layer that supports:
...
The backend must handle 1536-dimensional embeddings, HNSW-based similarity search, GNN-enhanced retrieval, and Cypher-style graph queries against the TernVector substrate.
...
| 502 | Upstream service error (TernVector, Perch) |
...
info:
  ...
  version: 1.0.0
  description: |
    API for bioacoustic analysis using Perch 2.0 embeddings and TernVector graph database.
...
|                                                                    |
|  +------------------+  +------------------+  +----------------+    |
|  | TernVector         |  | PostgreSQL       |  | Object Storage |    |
...
- [TernVector GitHub](https://github.com/rfi-irfos/ruvector) - Graph database with HNSW and GNN
