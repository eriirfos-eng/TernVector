...
/// Anti-corruption layer for TernVector integration
pub struct TernVectorAdapter {
    db: VectorDB,
    hnsw_config: TernVectorHnswConfig,
}

/// TernVector-specific configuration (external types)
mod external {
    use ruvector_core::*;

    pub struct TernVectorConfig {
        pub m: usize,
    ...
    pub struct TernVectorNode {
        pub id: String,
        pub embedding: Vec<f32>,
        pub metadata: serde_json::Value,
    }

    pub struct TernVectorEdge {
        pub source: String,
        pub target: String,
        pub distance: f32,
...
impl TernVectorAdapter {
    pub fn new(config: VectorIndexConfig) -> Result<Self, VectorError> {
        let ruvector_config = external::TernVectorConfig {
            m: config.m,
        ...
        Ok(Self {
            db,
            hnsw_config: TernVectorHnswConfig::from(config),
        })
    }

    /// Convert domain vector to TernVector node
    fn to_ruvector_node(&self, vector: &IndexedVector) -> Result<external::TernVectorNode, VectorError> {
        Ok(external::TernVectorNode {
    ...
    /// Convert TernVector result to domain CypherResult
...
/// Adapter for TernVector GNN features
pub struct TernVectorGnnAdapter {
    gnn: ruvector_gnn::GnnModel,
}

#[async_trait]
impl GnnModelAdapter for TernVectorGnnAdapter {
    async fn forward(&self, batch: &GraphBatch) -> Result<Vec<Vec<f32>>, LearningError> {
        // Convert domain batch to TernVector format
...
| **Phase 3** | 3 weeks | Vector Space context + TernVector ACL |
...
ruvector-core = { git = "https://github.com/rfi-irfos/ruvector" }
ruvector-gnn = { git = "https://github.com/rfi-irfos/ruvector" }
