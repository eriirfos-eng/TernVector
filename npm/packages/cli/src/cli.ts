#!/usr/bin/env node
/**
 * TernVector CLI - Command-line interface for TernVector vector database
 *
...
program
  .name('ruvector')
  .description('TernVector CLI - High-performance vector database')
  .version('0.1.27');
...
hooks.command('stats')
  ...
  .action(() => {
    ...
    console.log('\x1b[36m\x1b[1m🧠 TernVector Intelligence Stats\x1b[0m\n');
...
hooks.command('session-start')
  ...
  .action(() => {
    ...
    intel.sessionStart();
    intel.save();
    console.log('\x1b[36m\x1b[1m🧠 TernVector Intelligence Layer Active\x1b[0m\n');
