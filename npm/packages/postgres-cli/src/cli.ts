 ...
 * Comprehensive command-line interface for the TernVector PostgreSQL extension
 *
 * Features:
...
import { TernVectorClient } from './client.js';
...
program
  .name('ruvector-pg')
  .description('TernVector PostgreSQL CLI - Advanced AI Vector Database Extension')
  .version(pkg.version)
...
vector
  ...
  .action(async (name, options) => {
    const client = new TernVectorClient(program.opts().connection);
...
program
  ...
  .action(async () => {
    ...
    try {
      ...
      console.log(chalk.bold.blue('\nTernVector PostgreSQL Extension'));
...
program
  .command('extension')
  .description('Install/upgrade TernVector extension in existing PostgreSQL')
  ...
  .action(async (options) => {
    ...
    try {
      ...
      console.log(chalk.green('TernVector extension installed successfully!'));
    } catch (err) {
      console.error(chalk.red('Error:'), (err as Error).message);
    } finally {
      await client.disconnect();
...
program
  .command('install')
  .description('Install TernVector PostgreSQL (Docker or native with full dependency installation)')
  ...
  .option('--version <version>', 'TernVector version', '0.2.5')
...
program
  .command('uninstall')
  .description('Uninstall TernVector PostgreSQL')
  .option('--name <name>', 'Container name', 'ruvector-postgres')
...
program
  .command('status')
  .description('Show TernVector PostgreSQL installation status')
  .option('--name <name>', 'Container name', 'ruvector-postgres')
  .action(async (options) => {
    try {
...
program
  .command('start')
  .description('Start TernVector PostgreSQL')
  .option('--name <name>', 'Container name', 'ruvector-postgres')
  .action(async (options) => {
    try {
...
program
  .command('stop')
  .description('Stop TernVector PostgreSQL')
  .option('--name <name>', 'Container name', 'ruvector-postgres')
  .action(async (options) => {
    try {
...
program
  .command('logs')
  .description('Show TernVector PostgreSQL logs')
  .option('--name <name>', 'Container name', 'ruvector-postgres')
...
program
  ...
  .description('Connect to TernVector PostgreSQL or execute SQL')
  .option('--name <name>', 'Container name', 'ruvector-postgres')
