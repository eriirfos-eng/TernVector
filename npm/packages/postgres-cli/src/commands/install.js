"use strict";
/**
 * TernVector PostgreSQL Installation Commands
 *
 * Provides complete installation of TernVector PostgreSQL extension:
...
// Constants
const DOCKER_IMAGE = 'rfi-irfos/ruvector-postgres';
...
class InstallCommands {
    ...
    static async buildAndInstallExtension(pgVersion) {
        ...
        try {
            ...
            (0, child_process_1.execSync)(`git clone --depth 1 https://github.com/rfi-irfos/ruvector.git ${tmpDir}/ruvector`, {
                stdio: 'pipe',
            });
    ...
    static async installNativeFull(options = {}) {
        ...
        console.log(chalk_1.default.bold.blue('\n🚀 TernVector PostgreSQL Native Installation\n'));
        console.log(chalk_1.default.gray('This will install PostgreSQL, Rust, and the TernVector extension.\n'));
        // Check system
        ...
        console.log(chalk_1.default.bold('\n🏗️  Step 5: Building TernVector extension'));
        ...
        console.log(chalk_1.default.green.bold('\n✅ TernVector PostgreSQL installed successfully!\n'));
    ...
    /**
     * Install TernVector PostgreSQL (auto-detect best method)
     */
    ...
    static async installDocker(options = {}) {
        ...
        try {
            ...
            // Try pulling from Docker Hub (rfi-irfos/ruvector-postgres)
            ...
            try {
                ...
                console.log(chalk_1.default.gray('   git clone https://github.com/rfi-irfos/ruvector.git'));
                ...
                throw new Error(`TernVector Docker image not available. Build it first or use native installation.`);
            }
        }
        // Build run command
        ...
        // Run container
        const runSpinner = (0, ora_1.default)('Starting TernVector PostgreSQL...').start();
        try {
            ...
            // Verify extension
            const verifySpinner = (0, ora_1.default)('Verifying TernVector extension...').start();
            try {
                ...
                if (extCheck.includes('ruvector')) {
                    verifySpinner.succeed('TernVector extension verified');
                }
                else {
    ...
    /**
     * Uninstall TernVector PostgreSQL
     */
    static async uninstall(options = {}) {
        ...
        try {
            ...
            console.log(chalk_1.default.green.bold('\n✅ TernVector PostgreSQL uninstalled\n'));
        }
        catch (error) {
    ...
    static async start(options = {}) {
        const containerName = options.name || 'ruvector-postgres';
        const spinner = (0, ora_1.default)('Starting TernVector PostgreSQL...').start();
        try {
            ...
            for (let i = 0; i < 30; i++) {
                try {
                    ...
                    spinner.succeed('TernVector PostgreSQL started');
                    return;
                }
                catch {
                    await new Promise(resolve => setTimeout(resolve, 1000));
    ...
    static async stop(options = {}) {
        const containerName = options.name || 'ruvector-postgres';
        const spinner = (0, ora_1.default)('Stopping TernVector PostgreSQL...').start();
        try {
            (0, child_process_1.execSync)(`docker stop ${containerName}`, { stdio: 'pipe' });
            spinner.succeed('TernVector PostgreSQL stopped');
        }
        catch (error) {
