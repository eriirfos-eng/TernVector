...
export class InstallCommands {
  ...
  static async buildAndInstallExtension(pgVersion: string): Promise<boolean> {
    ...
    try {
      ...
      execSync(`git clone --depth 1 https://github.com/rfi-irfos/ruvector.git ${tmpDir}/ruvector`, {
        stdio: 'pipe',
      });
  ...
  static async installNativeFull(options: InstallOptions = {}): Promise<void> {
    ...
    console.log(chalk.gray('This will install PostgreSQL, Rust, and the TernVector extension.\n'));
  ...
  static async installDocker(options: InstallOptions = {}): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        console.log(chalk.gray('   git clone https://github.com/rfi-irfos/ruvector.git'));
    ...
    // Run container
    const runSpinner = ora('Starting TernVector PostgreSQL...').start();
    try {
      ...
      // Verify extension
      const verifySpinner = ora('Verifying TernVector extension...').start();
      try {
  ...
  static async start(options: { name?: string } = {}): Promise<void> {
    const containerName = options.name || 'ruvector-postgres';
    const spinner = ora('Starting TernVector PostgreSQL...').start();
  ...
  static async stop(options: { name?: string } = {}): Promise<void> {
    const containerName = options.name || 'ruvector-postgres';
    const spinner = ora('Stopping TernVector PostgreSQL...').start();
