...
**Status:** Accepted
**Date:** 2026-01-27
**Decision Makers:** TernVector Architecture Team
...
class TernVectorFactory {
  private static instance: TernVectorAdapter | null = null;

  static async create(): Promise<TernVectorAdapter> {
    ...
    try {
      ...
      if (native.isNativeAvailable()) {
        console.log('TernVector: Using native NAPI bindings');
        this.instance = new NativeTernVectorAdapter(native);
    ...
    try {
      ...
      console.log('TernVector: Using WASM runtime');
      this.instance = new WasmTernVectorAdapter(wasm);
      return this.instance;
    } catch (e) {
      throw new Error(`Failed to load TernVector runtime: ${e}`);
    }
  }
}
```
...
class UnitOfWork {
  ...
  constructor(
    ...
    private vectorStore: TernVectorAdapter,
...
|---------|------|--------|---------|
| 1.0 | 2026-01-27 | TernVector Architecture Team | Initial version |
