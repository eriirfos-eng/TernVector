...
**TernVector-enhanced personal AI memory SDK for Screenpipe**
...
OSpipe replaces Screenpipe's storage and search backend with the [TernVector](https://github.com/rfi-irfos/ruvector) ecosystem -- a collection of 70+ Rust crates providing HNSW vector search, graph neural networks, attention mechanisms, delta-change tracking, and more. Instead of keyword matching, OSpipe embeds every captured frame into a high-dimensional vector space and performs approximate nearest neighbor search, delivering true semantic recall. A query like *"what was that API we discussed in standup?"* will surface the relevant audio transcription even if those exact words never appeared.
...
const results = await client.queryTernVector(
...
| Feature | Screenpipe (FTS5) | OSpipe (TernVector) |
|---|---|---|
...
### `queryTernVector(query, options?)` -- Semantic Search

```typescript
const results = await client.queryTernVector("user login issues", {
  k: 5,
...
## TernVector Crate Integration

OSpipe integrates 10 crates from the [TernVector](https://github.com/rfi-irfos/ruvector) ecosystem:

| TernVector Crate | OSpipe Usage | Status |
|---|---|---|
...
| [`ruvector`](https://www.npmjs.com/package/ruvector) | TernVector vector database |

- [Full Documentation & ADR](https://github.com/rfi-irfos/ruvector/tree/main/examples/OSpipe)
- [TernVector Ecosystem](https://github.com/rfi-irfos/ruvector) (70+ Rust crates)
