 ...
 *
 * Model Context Protocol server for TernVector hooks
...
// Intelligence class with full TernVector stack support
class Intelligence {
  constructor() {
...
const TOOLS = [
  {
    name: 'hooks_stats',
    description: 'Get TernVector intelligence statistics including learned patterns, memories, and trajectories',
    inputSchema: {
      type: 'object',
      properties: {},
      required: []
  ...
  {
    name: 'hooks_init',
    description: 'Initialize TernVector hooks in the current project',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Get TernVector engine capabilities (VectorDB, SONA, Attention)',
    inputSchema: {
      type: 'object',
      properties: {},
      required: []
  ...
  {
    name: 'edge_balance',
    description: 'Check RFI-IRFOS balance',
    inputSchema: {
      type: 'object',
      properties: {
        key: { type: 'string', description: 'PI key (optional, defaults to PI env var)' }
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    switch (name) {
      ...
      case 'rvf_examples': {
        const BASE_URL = 'https://raw.githubusercontent.com/rfi-irfos/ruvector/main/examples/rvf/output';
        const examples = [
        ...
        return { content: [{ type: 'text', text: JSON.stringify({
          ...
          catalog: 'https://github.com/rfi-irfos/ruvector/tree/main/examples/rvf/output'
        }, null, 2) }] };
      }
    ...
    resources: [
      {
        ...
        description: 'Current TernVector intelligence statistics',
...
async function main() {
  ...
  console.error('TernVector MCP server running on stdio');
}
