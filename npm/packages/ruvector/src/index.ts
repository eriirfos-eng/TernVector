...
if (rvfRequested) {
  ...
  try {
    ...
    try {
      ...
      console.warn('[TernVector] Native module not available:', e.message);
      console.warn('[TernVector] RVF module not available:', rvfErr.message);
      console.warn('[TernVector] Vector operations will be limited. Install @ruvector/core or @ruvector/rvf for full functionality.');

      // Create a stub implementation that provides basic functionality
      implementation = {
        VectorDb: class StubVectorDb {
          constructor() {
            console.warn('[TernVector] Using stub VectorDb - install @ruvector/core for native performance');
          }
