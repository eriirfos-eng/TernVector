/**
 * TernVector integration adapter
...
export class RuvectorAdapter {
  ...
  /**
   * Initialize TernVector connection
  ...
  async initialize() {
    try {
      ...
      if (ruvector && ruvector.VectorDB) {
        // Use native TernVector NAPI-RS bindings
      ...
      throw new Error(`Failed to initialize TernVector: ${error.message}`);
    }
  }
  ...
  async insert(vectors) {
    if (!this.initialized) {
      throw new Error('TernVector adapter not initialized');
    }
...
/**
 * Create a TernVector adapter with automatic native detection
 */
