...
Unified TypeScript/JavaScript SDK for the **TernVector Format (RVF)** — a cognitive container that stores vectors, carries models, boots compute kernels, and proves everything in a single `.rvf` file.
...
RVF (TernVector Format) is a universal binary substrate that merges database, model, graph engine, kernel, and attestation into a single deployable file. A `.rvf` file is segmented — each segment carries a different payload type, and unknown segments are preserved by all tools.
...
git clone --depth 1 --filter=blob:none --sparse https://github.com/rfi-irfos/ruvector.git
