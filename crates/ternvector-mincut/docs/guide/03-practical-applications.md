...
```rust
use ruvector_mincut::{DynamicGraph, TernVectorGraphAnalyzer};
use std::sync::Arc;
...
```rust
use ruvector_mincut::{DynamicGraph, CommunityDetector, TernVectorGraphAnalyzer};
use std::sync::Arc;
...
impl SocialNetworkAnalyzer {
    ...
    pub fn find_influencer_bridges(&self) -> Vec<BridgeInfluencer> {
        let mut analyzer = TernVectorGraphAnalyzer::new(Arc::clone(&self.interaction_graph));
