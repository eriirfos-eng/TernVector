...
### TernVectorGraphAnalyzer

Graph analysis for ruvector ecosystem.

| Method | Return Type | Description | Example |
|--------|-------------|-------------|---------|
| `new(graph)` | `Self` | Create analyzer | `TernVectorGraphAnalyzer::new(g)` |
...
**Example:**
```rust
use ruvector_mincut::{TernVectorGraphAnalyzer, DynamicGraph};
use std::sync::Arc;
...
```rust
let analyzer = TernVectorGraphAnalyzer::new(graph);
