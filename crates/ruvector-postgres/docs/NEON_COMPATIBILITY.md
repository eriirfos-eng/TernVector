...
TernVector-Postgres is designed with first-class support for Neon's serverless PostgreSQL platform. This guide covers deployment, configuration, and optimization for Neon environments.
...
| Neon Feature | TernVector Support | Notes |
|--------------|------------------|-------|
...
Neon compute nodes are ephemeral and may be replaced at any time. TernVector-Postgres handles this by:
...
Critical for scale-to-zero. TernVector-Postgres achieves sub-100ms initialization:
...
Neon compute instances have memory limits based on compute units (CU). TernVector-Postgres is memory-conscious:
...
Neon restricts background workers for resource management. TernVector-Postgres is designed without them:
...
Neon uses PgBouncer in **transaction mode** for connection pooling. TernVector-Postgres is fully compatible:
...
- TernVector requires custom extension approval
...
```
Subject: Custom Extension Request - TernVector-Postgres

Body:
I would like to install the TernVector-Postgres extension for vector similarity search.
...
Documentation: https://github.com/rfi-irfos/ruvector/tree/main/crates/ruvector-postgres
...
-- TernVector provides migration path
-- (See MIGRATION.md)
```

## Migration from pgvector

TernVector-Postgres is API-compatible with pgvector. Migration is seamless:
...
## Neon Branching with TernVector
...
- **TernVector Issues**: https://github.com/rfi-irfos/ruvector/issues
