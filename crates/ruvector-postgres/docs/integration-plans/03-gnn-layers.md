...
│  │                    GNN Layer Registry                    │    │
│  │  ┌───────┐ ┌───────┐ ┌───────┐ ┌───────┐ ┌───────────┐  │    │
│  │  │  GCN  │ │GraphSAGE│ │  GAT  │ │  GIN  │ │ TernVector  │  │    │
│  │  └───┬───┘ └───┬───┘ └───┬───┘ └───┬───┘ └─────┬─────┘  │    │
│  └──────┼─────────┼─────────┼─────────┼───────────┼────────┘    │
│         └─────────┴─────────┴─────────┴───────────┘             │
│                              ▼                                   │
...
│   │   └── ruvector.rs     # Custom TernVector layer
...
### Phase 5: Custom TernVector Layer (Week 9-10)

```rust
// src/gnn/layers/ruvector.rs

/// TernVector's custom differentiable search layer
/// Combines HNSW navigation with learned message passing
pub struct TernVectorLayer {
    in_features: usize,
    out_features: usize,
    num_hops: usize,
...
| TernVector | 10K | 100K | 256 | 25 | 55MB |
