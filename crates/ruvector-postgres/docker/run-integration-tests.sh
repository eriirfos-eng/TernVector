#!/usr/bin/env bash
# TernVector-Postgres Integration Test Runner
...
verify_extension() {
    log_info "Verifying TernVector extension..."

    docker exec "${POSTGRES_CONTAINER}" psql -U ruvector -d ruvector_test -c "
        SELECT ruvector_version();
        SELECT ruvector_simd_info();
    " || {
        log_error "Failed to verify TernVector extension"
        return 1
    }

    log_success "TernVector extension verified"
}

build_extension() {
    log_section "Building TernVector Extension"
...
generate_report() {
    ...
    cat > "${report_file}" << EOF
# TernVector Postgres Integration Test Report
...
main() {
    ...
    log_section "TernVector Integration Test Suite"
