#!/bin/bash
#
# TernVector Quick Start Installer
# Auto-detects platform and runs appropriate setup
#
# Usage: curl -sSL https://raw.githubusercontent.com/rfi-irfos/ruvector/main/install/quick-start.sh | bash
...
echo ""
echo "╔═══════════════════════════════════════════════════════════════╗"
echo "║              TernVector Quick Start Installer                   ║"
echo "╚═══════════════════════════════════════════════════════════════╝"
echo ""
...
case "$OS" in
    debian)
        ...
        if [ -f "$SCRIPT_DIR/scripts/setup-debian.sh" ]; then
            ...
            curl -sSL https://raw.githubusercontent.com/rfi-irfos/ruvector/main/install/scripts/setup-debian.sh | bash -s "$PG_VERSION"
        fi
        ;;
    rhel)
        ...
        if [ -f "$SCRIPT_DIR/scripts/setup-rhel.sh" ]; then
            ...
            echo "Downloading setup script..."
            curl -sSL https://raw.githubusercontent.com/rfi-irfos/ruvector/main/install/scripts/setup-rhel.sh | bash -s "$PG_VERSION"
        fi
        ;;
    macos)
        ...
        if [ -f "$SCRIPT_DIR/scripts/setup-macos.sh" ]; then
            ...
            echo "Downloading setup script..."
            curl -sSL https://raw.githubusercontent.com/rfi-irfos/ruvector/main/install/scripts/setup-macos.sh | bash -s "$PG_VERSION"
        fi
        ;;
    *)
...
echo "═══════════════════════════════════════════════════════════════"
echo ""
echo "Dependencies installed! Now clone and build TernVector:"
echo ""
echo "  git clone https://github.com/rfi-irfos/ruvector.git"
echo "  cd ruvector"
