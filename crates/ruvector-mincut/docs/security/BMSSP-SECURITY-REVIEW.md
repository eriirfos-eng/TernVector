...
### 4.1 @rfi-irfos/bmssp Package Integrity

**Package Analysis:**

| Attribute | Value | Risk |
|-----------|-------|------|
| npm package | `@rfi-irfos/bmssp` | Scoped package (trusted author) |
| Source repository | https://github.com/rfi-irfos/bmssp | Verify ownership |
...
npm audit signatures @rfi-irfos/bmssp

# 2. Verify WASM binary hash
sha256sum node_modules/@rfi-irfos/bmssp/bmssp.wasm
# Expected: [document expected hash in SECURITY.md]

# 3. Verify source matches binary
cd node_modules/@rfi-irfos/bmssp
...
{
  "dependencies": {
    "@rfi-irfos/bmssp": "1.0.0"
  },
  "overrides": {
    "@rfi-irfos/bmssp": "$@rfi-irfos/bmssp"
  }
}
...
```bash
# Using syft
syft dir:node_modules/@rfi-irfos/bmssp -o spdx-json > bmssp-sbom.json
```

2. **Verify Reproducible Build:**
```bash
# Clone source
git clone https://github.com/rfi-irfos/bmssp.git
cd bmssp
...
async function verifyBmsspWasm(): Promise<boolean> {
    const wasmBytes = await fetch('/node_modules/@rfi-irfos/bmssp/bmssp.wasm')
...
4. npm package: https://www.npmjs.com/package/@rfi-irfos/bmssp
5. TernVector Security Audit Report (2026-01-18)
