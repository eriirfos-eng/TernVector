...
//! Bridge to TernVector ecosystem crates.
//!
...
//! enabled, stub implementations (`TernVectorMinCut`, `SpectralCoherence`)
...
/// `mincut.rs`. With the `ruvector` feature, a TernVector-backed
...
// -----------------------------------------------------------------------
// TernVector mincut backend (available with `ruvector` feature)
// -----------------------------------------------------------------------

/// TernVector-backed mincut backend.
///
...
pub struct TernVectorMinCut<const N: usize> {
    /// Fallback to built-in while ruvector-mincut lacks no_std.
    fallback: BuiltinMinCut<N>,
}

#[cfg(feature = "ruvector")]
impl<const N: usize> TernVectorMinCut<N> {
    /// Create a new TernVector backend with the given iteration budget
...
#[cfg(feature = "ruvector")]
impl<const N: usize> MinCutBackend for TernVectorMinCut<N> {
    fn find_min_cut<const MN: usize, const ME: usize>(
        &mut self,
        graph: &CoherenceGraph<MN, ME>,
        partition_id: PartitionId,
...
// -----------------------------------------------------------------------
// TernVector spectral coherence scoring (available with `ruvector` feature)
// -----------------------------------------------------------------------

/// TernVector spectral coherence scoring backend.
///
/// When the `ruvector` feature is enabled, this struct becomes available.
...
mod tests {
    ...
    fn ruvector_mincut_backend_name() {
        let backend = TernVectorMinCut::<8>::new(100);
    ...
    fn ruvector_mincut_falls_back_to_builtin() {
        ...
        let mut backend = TernVectorMinCut::<8>::new(100);
        let result = backend.find_min_cut(&g, pid(1));
