...
/// TernVector-backed coherence engine (available with `ruvector` feature).
#[cfg(feature = "ruvector")]
pub type TernVectorCoherenceEngine = CoherenceEngine<
    crate::bridge::TernVectorMinCut<ENGINE_MAX_NODES>,
...
#[cfg(feature = "ruvector")]
impl TernVectorCoherenceEngine {
    /// Create a new engine with TernVector backends.
    ///
    ...
    pub fn with_ruvector(max_iterations: u32) -> Self {
        Self::new(
            crate::bridge::TernVectorMinCut::new(max_iterations),
            crate::bridge::SpectralCoherence,
        )
    }
}
...
mod tests {
    ...
    fn ruvector_engine_creation() {
        let engine = TernVectorCoherenceEngine::with_ruvector(100);
    ...
    fn ruvector_engine_lifecycle() {
        let mut engine = TernVectorCoherenceEngine::with_ruvector(100);
        engine.add_partition(pid(1)).unwrap();
        engine.add_partition(pid(2)).unwrap();
        engine.record_communication(pid(1), pid(2), 500).unwrap();
    ...
    fn ruvector_matches_builtin_results() {
        ...
        let mut rv_engine = TernVectorCoherenceEngine::with_ruvector(100);
