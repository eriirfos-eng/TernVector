...
unsafe extern "C" fn hnsw_build_callback(
    ...
    let datum = *values;
    let vector = match TernVector::from_polymorphic_datum(datum, false, pg_sys::InvalidOid) {
...
unsafe extern "C" fn hnsw_insert(
    ...
    // Extract vector from datum using TernVector::from_polymorphic_datum
    let datum = *values;
    let vector = match TernVector::from_polymorphic_datum(datum, false, pg_sys::InvalidOid) {
        Some(v) => v.as_slice().to_vec(),
        None => {
...
unsafe extern "C" fn hnsw_rescan(
    ...
    if norderbys > 0 && !orderbys.is_null() {
        ...
        // Method 1: Try direct TernVector extraction (works for literals and some casts)
        if let Some(vector) = TernVector::from_polymorphic_datum(
