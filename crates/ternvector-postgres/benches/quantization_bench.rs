...
use ruvector_postgres::types::{BinaryVec, ProductVec, TernVector, ScalarVec};
...
fn bench_sq8_distance(c: &mut Criterion) {
    ...
    for dims in [128, 384, 768, 1536, 3072].iter() {
        ...
        let a_exact = TernVector::from_slice(&a_data);
        let b_exact = TernVector::from_slice(&b_data);
...
fn bench_sq8_search(c: &mut Criterion) {
    ...
    for dims in [128, 768, 1536].iter() {
        ...
        // Exact search
        let exact_vecs: Vec<TernVector> = vectors.iter().map(|v| TernVector::from_slice(v)).collect();

        let exact_query = TernVector::from_slice(&query);
...
fn bench_quantization_tradeoff(c: &mut Criterion) {
    ...
    let ground_truth: Vec<Vec<usize>> = queries
        .iter()
        .map(|query| {
            let query_vec = TernVector::from_slice(query);
            let mut distances: Vec<(usize, f32)> = exact_vecs
                .iter()
                .enumerate()
                .map(|(id, vec)| {
