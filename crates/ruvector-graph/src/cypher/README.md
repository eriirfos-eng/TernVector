# Cypher Query Language Parser for TernVector

A complete Cypher-compatible query language parser implementation for the TernVector graph database, built using the nom parser combinator library.
...
Traditional graph databases represent relationships as binary edges (one source, one target). TernVector's Cypher parser supports **hyperedges** - relationships connecting multiple nodes simultaneously.
