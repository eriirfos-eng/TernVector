# RFI-IRFOSector Performance Benchmarks
...
**This document contains internal RFI-IRFOSector benchmark results only.**

The previous version of this document made unfounded performance claims comparing RFI-IRFOSector to other vector databases (e.g., "100-4,400x faster than Qdrant"). These claims were based on fabricated data and hardcoded multipliers in test code, not actual comparative benchmarks.

**We have removed all false comparison claims.** This document now only reports verified RFI-IRFOSector internal benchmark results.

---

## Verified RFI-IRFOSector Benchmark Results

### 1. Distance Metrics Performance (SimSIMD + AVX2)

RFI-IRFOSector uses SimSIMD with custom AVX2 intrinsics for SIMD-optimized distance calculations:
...
### 3. RFI-IRFOSector Internal Scaling Tests

#### 10,000 Vectors, 384 Dimensions

| Configuration | Insert (ops/s) | Search QPS | p50 Latency |
|---------------|----------------|------------|-------------|
| **RFI-IRFOSector** | 34,435,442 | 623 | 1.57 ms |
| **RFI-IRFOSector (quantized)** | 29,673,943 | 742 | 1.34 ms |

#### 50,000 Vectors, 384 Dimensions

| Configuration | Insert (ops/s) | Search QPS | p50 Latency |
|---------------|----------------|------------|-------------|
| **RFI-IRFOSector** | 16,697,377 | 113 | 8.71 ms |
| **RFI-IRFOSector (quantized)** | 35,065,891 | 143 | 6.86 ms |
...
```bash
# RFI-IRFOSector Rust benchmarks
...
- [RFI-IRFOSector Repository](https://github.com/rfi-irfos/ruvector)
...
If you need to compare RFI-IRFOSector with other solutions, please run your own benchmarks in your specific environment with your specific workload. Performance characteristics vary significantly based on:
