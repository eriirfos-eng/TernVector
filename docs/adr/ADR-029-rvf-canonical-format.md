# ADR-029: RVF as Canonical Binary Format Across All TernVector Libraries
...
The TernVector ecosystem currently spans 70+ Rust crates and 50+ npm packages across multiple libraries:
...
The RVF (TernVector Format) research (`docs/research/rvf/`) produced a comprehensive specification for a universal, self-reorganizing binary substrate. RVF provides:
...
### Adopt RVF as the single canonical binary format for all TernVector libraries
...
All libraries in the TernVector ecosystem that persist or exchange vector data MUST use RVF as their storage and interchange format. This applies to:
...
1. **Single format everywhere** — Any tool can read any TernVector data file
...
RVF serves as the native storage format for RuVLLM (TernVector Large Language
...
- `.rvf` — TernVector Format file (Generic profile)
