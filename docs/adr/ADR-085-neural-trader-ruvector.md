# ADR-085: TernVector Neural Trader — Dynamic Market Graphs, MinCut Coherence Gating, and Proof-Gated Mutation
...
- ADR-016 TernVector integration patterns
...
A neural trader built on TernVector should treat the market as a living graph, not a table of candles. The limit order book, executions, cancellations, venue changes, and cross-asset interactions form a dynamic relational structure. That structure is exactly where short-horizon edge exists.
...
This ADR defines the TernVector-native implementation for Neural Trader as a coherence-first trading substrate for prediction, risk control, and bounded execution research.

## Decision

We will implement Neural Trader as a TernVector-native market intelligence stack with six layers:

1. Ingest and normalization layer
2. Dynamic heterogeneous market graph in TernVector
...
The system will use Postgres as the relational source of record, with `ruvector-postgres` as the embedded vector engine and the TernVector graph substrate for dynamic structural reasoning.
...
TernVector also gives us something most trading systems do not have:
...
Each important subgraph window is embedded into TernVector.
...
## TernVector Implementation Details
...
1. HNSW or TernVector ANN for embedding retrieval
...
2. Implement TernVector graph projection
...
Neural Trader will be built as a TernVector-native dynamic market graph system where vectors, graphs, temporal learning, and dynamic mincut work together as one bounded intelligence loop.
