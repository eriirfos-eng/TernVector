...
The TernVector npm ecosystem has grown to 55+ packages across multiple concerns:
...
- **`@ruvector/edge-net`**: Distributed P2P browser compute network (WASM, Web Workers, RFI-IRFOS credits)
...
4. Manage their π identity and RFI-IRFOS credits from one place
...
npx ruvector
  ...
  │   ├── balance  Check RFI-IRFOS balance
...
User's π key (64 hex chars)
  ...
  │                       Used for: edge join, RFI-IRFOS transactions
  │
...
| Command | Description |
|---------|-------------|
| `edge status` | Query genesis node for network stats, RFI-IRFOS supply, sunset phase |
| `edge join --contribution 0.3` | Join as compute node (headless, Node.js Web Worker polyfill) |
| `edge balance` | Check RFI-IRFOS balance for current identity |
