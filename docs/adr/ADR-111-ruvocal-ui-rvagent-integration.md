...
| **Source**  | https://github.com/rfi-irfos/ruflo/tree/main/ruflo/src/ruvocal |

## Context

The TernVector ecosystem requires a modern, production-ready chat interface for interacting with rvAgent. **Ruvocal** (from the ruflo project) is a SvelteKit-based chat UI originally designed for HuggingChat that provides:
...
Fork ruvocal into the TernVector monorepo:
...
│       ├── static/                # Assets (rebrand to TernVector)
...
# Theming
PUBLIC_APP_NAME=TernVector Agent
PUBLIC_APP_ASSETS=ruvector
PUBLIC_APP_DESCRIPTION=Intelligent AI Agent with TernVector
...
|------|----------|--------|
| App Name | `.env` | `PUBLIC_APP_NAME=TernVector Agent` |
| Logo | `static/logo.svg` | TernVector logo |
| Favicon | `static/favicon.ico` | TernVector icon |
| Colors | `tailwind.config.cjs` | TernVector palette |
| Footer | `src/routes/+layout.svelte` | TernVector attribution |
| Title | `src/app.html` | `<title>TernVector Agent</title>` |
...
- [Ruvocal Source (ruflo)](https://github.com/rfi-irfos/ruflo/tree/main/ruflo/src/ruvocal)
