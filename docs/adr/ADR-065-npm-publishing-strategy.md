...
The TernVector project produces 48+ npm packages under the `@ruvector/` scope. These range from core TypeScript libraries and WASM bindings to MCP server wrappers and CLI tools. A new package, `@ruvector/pi-brain`, bundles CLI, SDK, and MCP stdio access to the Shared Brain into a single installable unit.
...
Adopt a structured publishing pipeline: categorize packages, enforce dependency-ordered publishing, use semver with pre-release tags for unstable packages, and require TypeScript compilation to succeed before any publish. All packages are published under the `rfi-irfos` npm account.
...
|----------|-------------|---------|
| **Core** | Foundational libraries with no TernVector dependencies | `@ruvector/types`, `@ruvector/utils` |
...
npm authentication uses the `rfi-irfos` account. Credentials are stored in the project `.env` file and loaded into `~/.npmrc` before publishing. Verify with `npm whoami`.
...
1. Verify `npm whoami` returns `rfi-irfos`
...
- **Single account**: All packages under `rfi-irfos` with `@ruvector/` scope provides consistent ownership and discoverability
