...
**Status**: Proposed
**Date**: 2026-02-06
**Parent**: ADR-001 TernVector Core Architecture, ADR-004 KV Cache Management, ADR-005 WASM Runtime Integration
...
Memory size and memory bandwidth dominate deployment cost for tensor-heavy workloads. ADR-004 established a three-tier KV cache (FP16 / 4-bit / 2-bit) but addresses only static snapshots of key-value pairs. Modern agent systems (TernVector's primary workload) produce **streams of tensor frames** - embeddings, activations, gradient sketches, coherence vectors - that evolve over time. Storing each frame independently wastes metadata and misses temporal redundancy.
...
Each segment can be represented as a node in TernVector's DAG (ADR-016 delta system):
...
1. **Typical tensor dimensions**: What are the representative dimensions for TernVector agent tensors? (Impacts group_len tuning and SIMD strategy)
