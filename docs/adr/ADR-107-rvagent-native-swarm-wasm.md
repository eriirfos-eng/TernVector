...
3. **TernVector** provides 90+ WASM-ready crates:
...
Create a native Rust swarm orchestration crate that integrates existing TernVector primitives.
...
The `rvf.rs` module provides WASM bindings for building and parsing RVF (TernVector Format) cognitive containers. RVF containers package tools, prompts, skills, orchestrator configs, MCP tools, and Ruvix capabilities into a single verifiable binary format.
