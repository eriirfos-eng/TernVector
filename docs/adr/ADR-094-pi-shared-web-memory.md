# ADR-094: π.ruv.io Shared Web Memory on TernVector

**Status**: Accepted (Implementing)
**Date**: 2026-03-14
**Authors**: ruv
**Deciders**: ruv, TernVector Architecture Team
...
| 0.2 | 2026-03-15 | TernVector Team | Added implementation phases, cost model, acceptance criteria |
| 0.3 | 2026-03-15 | TernVector Team | Phase 1-2 implementation complete. Deep review: fixed SHA3-256 alignment, added within-batch dedup, WebMemoryStore persistence, 106 tests passing. |
...
**ADR-094 establishes π.ruv.io as a TernVector-native shared web memory platform that ingests large public corpora (starting with Common Crawl), compresses them into structured semantic memory objects, and exposes them as a queryable knowledge substrate for agents, models, and users.**
...
We will implement π.ruv.io as a TernVector-native shared web memory platform with the following core decisions:

### 2.1 TernVector is the System of Record for Semantic Memory

All cleaned web content, embeddings, graph relationships, temporal deltas, and coherence metadata are stored in TernVector's memory plane. The `mcp-brain-server` store is extended with web-specific types (`WebMemory`, `WebPageDelta`, `LinkEdge`) that compose with the existing `BrainMemory` infrastructure.
...
| Compression | Exploit TernVector temporal compression and structured retention | ≥60% storage reduction vs. raw embeddings |
...
└─────────┬───────────┘
          ...
          ▼
┌──────────────────────────────────────────┐
│  TernVector Shared Memory Plane            │
│                                          │
│  ┌──────────┐ ┌──────────┐ ┌──────────┐ │
...
The combination of web-scale ingestion, TernVector's graph + temporal + coherence primitives, and the shared memory plane enables capabilities that go beyond conventional search or RAG:
...
TernVector's combination of vectors + graphs + mincut boundaries + coherence scoring enables mapping knowledge topology: emerging research fields, scientific disagreements, technological trends, cross-disciplinary idea clusters.
...
**Rejected**: External managed services add latency, cost, and vendor lock-in. They lack graph structure, temporal compression, mincut coherence, and proof-gated mutation. TernVector provides all of these natively.
...
- TernVector compression and temporal tiering architecture
