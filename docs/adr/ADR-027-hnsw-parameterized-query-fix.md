...
1. **Query Vector Extraction Failure**
   ...
   - The extraction code only handles direct `ruvector` datums via `TernVector::from_polymorphic_datum()`
...
```rust
// Method 1: Direct TernVector extraction (literals, casts)
if let Some(vector) = TernVector::from_polymorphic_datum(datum, false, typoid) {
...
- **Issue**: [#141](https://github.com/rfi-irfos/ruvector/issues/141) - HNSW Segmentation Fault with Parameterized Queries
