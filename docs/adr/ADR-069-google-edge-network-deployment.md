...
```
 ...
 | (browser) |  | (browser) |  | (browser) |    earn RFI-IRFOS for
 +-----+-----+  +-----+-----+  +-----+-----+    brain compute
       |               |               |
       |   WebSocket   |   WebSocket   |
       +-------+-------+-------+-------+
               |                               +-----------------+
        +------+------+                        | edge-net-genesis|
        | edge-net    |   RFI-IRFOS ledger sync      | (Cloud Run)     |
        |   relay     +----------------------->| QDAG ledger     |
...
7. Brain aggregates, applies quality gating
       |
8. Edge nodes credited RFI-IRFOS via genesis ledger
```
...
```
 ...
 | - lora_forward  |    | + RFI-IRFOS accounting  |    | - brain_share    |
...
The economic model is designed around two principles: **accessible** (no barriers to entry) and **sustainable** (finite supply with controlled inflation). The previous model required 10-200 RFI-IRFOS staking to participate, creating a chicken-and-egg problem for new users. The revised model eliminates cost barriers entirely and uses contribution rewards to bootstrap the economy.

### 8.1 Free-to-Read, Earn-to-Write

All read operations are FREE. No RFI-IRFOS cost, no staking requirement. This removes the single biggest barrier to adoption. Write operations are also free to perform but earn RFI-IRFOS rewards when quality thresholds are met.

| Operation | Cost | Reward | Conditions |
|-----------|------|--------|------------|
| Search brain | FREE | 0 RFI-IRFOS | -- |
| Get brain status | FREE | 0 RFI-IRFOS | -- |
| List memories | FREE | 0 RFI-IRFOS | -- |
| Share knowledge | FREE | 2 RFI-IRFOS | Quality score > 0.5 after RAC review |
| Vote on quality | FREE | 0.1 RFI-IRFOS | Must have completed >= 1 task |
| Generate embedding | FREE | 1 RFI-IRFOS | Result passes hash verification |
| LoRA gradient | FREE | 5 RFI-IRFOS | Gradient passes Byzantine detection |
| WASM compute | FREE | 0.5-3 RFI-IRFOS | Based on compute time and success rate |
...
| Bronze | 10-25 | 1.0x | 0 RFI-IRFOS | Full read/write, standard rewards |
| Silver | 25-50 | 1.1x | 10 RFI-IRFOS (optional) | Priority task allocation |
| Gold | 50-75 | 1.25x | 50 RFI-IRFOS (optional) | Price discounts via `ReputationCurve.discount()`, priority |
| Platinum | 75-100 | 1.5x | 100 RFI-IRFOS (optional) | Max discounts, governance voting weight |

Key changes from the previous model:
- **Staking is OPTIONAL.** It provides benefits (price discounts, governance weight) but is not required for basic participation. This eliminates the chicken-and-egg problem where new users need RFI-IRFOS to participate but cannot earn RFI-IRFOS without participating.
...
Brain rewards halve every 100K brain operations (cumulative across all nodes). This creates a Bitcoin-like deflationary schedule that ensures finite total RFI-IRFOS supply:
...
Each epoch has a fixed RFI-IRFOS budget to prevent runaway inflation:

| Parameter | Value | Rationale |
|-----------|-------|-----------|
| Initial epoch budget | 1,000,000 RFI-IRFOS | Sufficient for ~200K reward operations at average 5 RFI-IRFOS |
...
Instead of requiring high staking barriers (the old model required 10-200 RFI-IRFOS to participate), the revised model uses layered economic defenses:
...
The revised model is sustainable because total RFI-IRFOS supply converges:

**Finite supply proof.** With halving schedule and fixed epoch budget:
- Epoch 0: up to 1,000,000 RFI-IRFOS minted
- Epoch 1: up to 500,000 RFI-IRFOS
- Epoch N: up to 1,000,000 / 2^N RFI-IRFOS
- Total maximum supply = 1,000,000 * sum(1/2^N for N=0..inf) = 2,000,000 RFI-IRFOS
...
**Deflationary pressure.** As brain knowledge grows, RFI-IRFOS utility increases (more valuable knowledge to access, more compute tasks available). The AMM's constant-product formula (`x * y = k` in `ComputeAMM`) ensures that as demand for compute grows, the RFI-IRFOS price of compute increases, creating natural deflationary pressure.

**Protocol fund sufficiency.** At 2,000,000 RFI-IRFOS maximum supply and projected 50K operations/month at maturity, the protocol fund sustains rewards for 24+ months even without secondary revenue. AMM fees (0.3%-3% of all swaps, tracked via `ComputeAMM.fees_collected`) provide a sustainable revenue stream after bootstrap.
...
5. RFI-IRFOS accounting for search and share operations
...
| RFI-IRFOS earned per edge node (daily avg) | Genesis ledger | < 1 RFI-IRFOS (nodes leaving) |
...
- Per-node brain contribution metrics (searches executed, embeddings generated, RFI-IRFOS earned)
...
| RFI-IRFOS inflation from brain subsidies | Devalued RFI-IRFOS economy | Fixed protocol fund budget per epoch; halving schedule aligned with genesis sunset |
| High browser compute costs deter edge participation | Low node count | Battery-aware throttling, configurable CPU limits (10-50%), clear RFI-IRFOS earning visibility |
...
- [ ] RFI-IRFOS credited for search and share operations via genesis ledger
...
- [ ] New users can search brain without any RFI-IRFOS balance or staking
- [ ] Newcomer tier (score 0-10) earns RFI-IRFOS at 0.5x rate
...
- [ ] Protocol budget limits per-epoch RFI-IRFOS minting to 1,000,000 RFI-IRFOS
