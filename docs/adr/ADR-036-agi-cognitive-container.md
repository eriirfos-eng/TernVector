# ADR-036: TernVector AGI Cognitive Container with Claude Code Orchestration

**Status**: Partially Implemented
**Date**: 2026-02-15 (updated 2026-02-17)
**Decision owners**: TernVector platform team, Claude Flow orchestration team, RVF runtime team
...
TernVector is the existential substrate. RVF is the organizational and packaging
...
| Term | Definition |
|------|-----------|
| **TernVector substrate** | Persistent world model combining vectors, graphs, constraints, and signals. Supports graph querying via Cypher. Includes self-learning and graph neural embedding updates, with dynamic minimum-cut as a coherence signal. |
...
| 3 | LLM + TernVector world model + RVF cognitive container, orchestrated by Claude Code and Claude Flow | **Selected** |
...
3. **TernVector** is the existential substrate, storing world model state, typed
...
4. **RVF** is the portable intelligence package format. It encapsulates the
   agent runtime, TernVector state snapshot and deltas, policies, indexes, tool
...
4. TernVector database cluster (or embedded instance)
...
6. **Commit and witness**: Deltas are committed into TernVector and sealed into
...
|--------|--------------|------------|
| **Who** | Claude Code + Claude Flow | TernVector + RVF |
...
| TernVector retrieval results | Proposed memory mutations |
...
| TernVector shared memory state | Aggregated plan; learning loop updates |

Architecture: router-to-swarm-to-agents with learning loop and shared memory.

### Component C: TernVector Substrate
...
- TernVector retrieval makes these skills easier to select
...
| **TernVector snapshot** | Base world model graph, indexes, embeddings, skill library, policy nodes |
...
| 2 | Policy as data | Policies live in TernVector and are embedded in RVF manifest; policy cannot silently change between runs |
...
**Deliverables:**
1. TernVector schema and APIs for events, claims, evidence, contradictions
2. RVF container manifest format for model, policy, tool registry, snapshots
3. MCP server exposing TernVector and RVF operations to Claude Code
...
- RFC 8032: Ed25519
- FIPS 180-4: SHA-256
- Dynamic minimum-cut (arXiv preprint referenced in TernVector mincut crate)
