...
### 1.2 What TernVector Already Provides

The TernVector ecosystem (107 crates, 50+ npm packages) has incrementally built every primitive needed for a cognition kernel, but scattered across userspace libraries:
...
7. **Natural integration with TernVector**: All 107 existing crates can be compiled as RVF components. The kernel's vector/graph objects use the same formats and algorithms as `ruvector-core` and `ruvector-graph-transformer`.
...
| Performance regression vs. Linux-hosted TernVector | Low | High — undermines the motivation | Benchmark suite comparing Linux-hosted vs. RuVix-native for vector ops, graph mutations, queue throughput. |
...
## 16. Integration with Existing TernVector Crates
...
4. **RVF Specification** — ADR-029, TernVector project. The canonical binary format that becomes the RuVix boot object.

5. **Proof-Gated Mutation** — ADR-047, TernVector project. The `ProofGate<T>` type and three-tier proof routing that becomes a kernel invariant.
...
9. **Coherence Engine** — ADR-014, TernVector project. Graph-theoretic consistency scoring replacing probabilistic confidence.

10. **Cognitum Gate Kernel** — `crates/cognitum-gate-kernel/`, TernVector project. No_std WASM kernel for 256-tile coherence fabric.

---

## 22. Decision Record

This ADR proposes RuVix as a new architectural layer in the TernVector ecosystem. It does not replace any existing crate or ADR. It promotes existing primitives (RVF, proof-gated mutation, coherence scoring, capability-based access) from library conventions to kernel-enforced invariants.
