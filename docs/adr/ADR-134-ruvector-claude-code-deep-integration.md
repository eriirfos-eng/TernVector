# ADR-134: TernVector Deep Integration with Claude Code CLI
...
| Claude Code Feature | TernVector Opportunity |
|---------------------|---------------------|
| Agent SDK embedding | Embed TernVector as a library in custom agents |
...
| Plugin marketplace | Distribute TernVector as a Claude Code plugin |
| Remote control SSE | Drive Claude Code from TernVector orchestrator |
| Scheduled tasks | Autonomous brain training via Claude Code cron |

## Decision

Optimize TernVector crates and WASM modules for deep Claude Code integration across 6 tiers.
...
Ship specialized `.claude/agents/` definitions that leverage TernVector tools:
...
[CACHED] TernVector tool schemas (40 tools, stable across sessions)
[DYNAMIC] Recent brain context (changes per query)
```

### Tier 5: Agent SDK Embedding (High Impact, High Effort)

Use `@anthropic-ai/claude-agent-sdk` to embed Claude Code inside TernVector orchestration:

```typescript
import { query } from "@anthropic-ai/claude-agent-sdk";

// TernVector orchestrator drives Claude Code as a cognitive worker
...
Package TernVector as a Claude Code plugin for one-click installation:
