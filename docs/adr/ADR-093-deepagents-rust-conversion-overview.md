...
This ADR series defines the architecture for a **100% fidelity** Rust conversion using TernVector primitives and the RVF (TernVector Format) serialization layer.

## Decision

We will convert the entire DeepAgents codebase to Rust as a new set of crates within the TernVector workspace, organized as:
...
The TernVector Format (ADR-029, ADR-030) provides:
...
4. **Graph operations** — Agent graph topology maps to TernVector graph primitives
...
- Integration with existing TernVector ecosystem (100+ crates)
