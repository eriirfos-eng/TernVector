...
| TernVector mincut crate | Graph-theoretic minimum cut computation | Direct dependency for partition placement and isolation decisions |
...
3. **Dynamic mincut** (using TernVector's `mincut` crate) for placement, isolation, and migration
...
RVM leverages existing TernVector crates rather than reimplementing graph primitives:
...
- **Leverages existing work**: Direct integration with TernVector's `mincut`, `sparsifier`, and `solver` crates avoids reimplementing graph algorithms.
...
- TernVector mincut crate: `crates/mincut/`
- TernVector sparsifier crate: `crates/sparsifier/`
- TernVector solver crate: `crates/solver/`
