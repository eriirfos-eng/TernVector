...
RVM is a Rust-first bare-metal microhypervisor that replaces the VM abstraction with **coherence domains** (partitions). It runs standalone without Linux or KVM, targeting QEMU virt as the reference platform with paths to real hardware on AArch64, RISC-V, and x86-64. The hypervisor integrates TernVector's `mincut`, `sparsifier`, and `solver` crates as first-class subsystems driving placement, isolation, and scheduling decisions.
...
11. [Integration with TernVector](#11-integration-with-ruvector)
...
## 11. Integration with TernVector
