...
TernVector is a 79-crate Rust monorepo (v2.0.3, edition 2021, `rust-version = "1.77"`) that
...
and `criterion`-based benchmarking. Its 9-crate workspace structure mirrors TernVector's
modular architecture. License compatibility is confirmed: MIT (TernVector) and
...
uses `nalgebra` while TernVector's crates use `ndarray`. The bridge layer resolves this
...
4. **Maintenance burden of external alignment**: The solver is actively developed
   ...
   - **Mitigation**: Vendor the core algorithm crate (`sublinear-solver-core`)
     into the workspace. Wrap it behind TernVector's own `SolverEngine` trait to
...
- **Cons**:
  ...
  - Cannot customize algorithms for TernVector-specific sparse patterns
...
Reimplement all sublinear algorithms from scratch within a new TernVector crate,
using only ndarray as the backend.

- **Pros**:
  - Complete control over implementation
  - No nalgebra dependency; ndarray-only backend
  - Tailored to TernVector's exact sparse matrix formats
- **Cons**:
...
algorithms, wraps them behind TernVector's own trait hierarchy (`SolverEngine`,
