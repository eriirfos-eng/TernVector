...
| **Status** | Accepted |
| **Date** | 2026-02-20 |
| **Authors** | TernVector Architecture Team |
...
TernVector possesses a sophisticated, multi-layered memory infrastructure designed for
...
### 1.1 Existing TernVector Memory Infrastructure

The following memory subsystems are already operational in the TernVector codebase:
...
combining TernVector's existing storage with solver overhead:
...
solver into TernVector's existing memory infrastructure.

### 2.1 Arena-Based Scratch Space for Solver Temporaries

All per-solve temporary allocations use TernVector's existing arena allocator from
...
/// Solver scratch space backed by TernVector's arena allocator.
...
The solver borrows vector data directly from TernVector's SoA storage and HNSW graph
...
The solver integrates with TernVector's compute budget system (from `prime-radiant`'s
...
The solver integrates with TernVector's observability infrastructure to provide
...
/// with TernVector's allocation strategy).
...
Let the solver manage its own heap allocations independently of TernVector.

- **Pros**: Simple implementation, no coupling to TernVector internals
...
Override nalgebra's default allocator with TernVector's arena allocator using
...
Integrate with all three layers of TernVector's memory infrastructure: arena for
...
4. TernVector Architecture Analysis (doc 05-architecture-analysis.md) --
   existing memory subsystem documentation
5. TernVector Performance Analysis (doc 08-performance-analysis.md) --
...
|---------|------|--------|---------|
| 1.0 | 2026-02-20 | TernVector Architecture Team | Initial proposal |
