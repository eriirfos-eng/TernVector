...
The error handling strategy must also account for the quantization layer: TernVector stores vectors at multiple precision levels (f32, u8, int4, binary), and solver output that will be quantized must track solver error and quantization error as separate budgets to ensure the combined error stays within the caller's epsilon.
...
TernVector stores vectors at multiple quantization levels. Solver error and quantization error must be tracked as separate budgets whose sum stays within the caller's tolerance.

```rust
/// Quantization tier used in TernVector.
...
- Quantization-aware error budgets prevent the combined solver + quantization error from exceeding caller tolerance, which is critical for TernVector's multi-tier quantization (u8, int4, binary)
...
- **Cons**: Unacceptable for production. A single NaN in a Neumann iteration would crash the entire TernVector process. No fallback, no partial results, no diagnostic information.

### Option 2: Return NaN/Infinity Sentinel Values

- **Pros**: Zero overhead. No error types needed. Caller checks `is_nan()`.
- **Cons**: Silent corruption risk. A NaN that propagates through GNN message-passing, attention scores, or coherence energy is extremely difficult to trace back to its origin. Violates TernVector's data integrity guarantees.
...
|---------|------|--------|---------|
| 0.1 | 2026-02-20 | TernVector Team | Initial proposal |
| 1.0 | 2026-02-20 | TernVector Team | Accepted: full implementation complete |
...
- TernVector Algorithm Analysis (document 10) -- sublinear algorithm mathematical foundations
- TernVector Architecture Analysis (document 05) -- layered integration strategy
