...
| `@ruvector/rvf` | 0.1.9 | TernVector Format SDK | Yes |
...
| `@rfi-irfos/strange-loop` ^0.3.0 | (none) | N/A | **NO OVERLAP**: Not present in ruvector |
...
| `@rfi-irfos/strange-loop` integration | Self-referential reasoning patterns | Novel capability not present in ruvector. Extends the self-learning architecture (SONA, EWC, Thompson Sampling) with recursive reasoning. |
...
| `@rfi-irfos/strange-loop` not in ruvector ecosystem | **LOW** | This is a transitive dependency of sublinear-time-solver only. No action needed unless ruvector wants to use it directly. |
