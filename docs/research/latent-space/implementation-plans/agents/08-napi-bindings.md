...
Create high-performance Node.js bindings for TernVector's GNN latent space attention mechanisms using NAPI-RS, enabling seamless integration with JavaScript/TypeScript applications.
...
description = "Node.js bindings for TernVector GNN latent space attention mechanisms"
license = "MIT"
...
/// Initialize the TernVector native module
#[napi]
pub fn init() -> Result<String> {
    Ok("TernVector native module initialized".to_string())
}
...
impl From<TernVectorError> for Error {
    fn from(err: TernVectorError) -> Self {
...
use crate::{AttentionConfig, AttentionOutput, TernVectorError};
...
impl DotProductAttention {
    ...
    pub fn compute_batch(
        ...
        if queries.len() != keys.len() || queries.len() != values.len() {
            return Err(Error::from(TernVectorError::InvalidInput(
    ...
    pub async fn compute_batch_async(
        ...
        for task in tasks {
            let result = task
                .await
                .map_err(|e| Error::new(Status::GenericFailure, e.to_string()))?
                .map_err(|e: TernVectorError| Error::from(e))?;
            results.push(result);
        }
    ...
    fn compute_internal(
        ...
        key: Vec<Vec<f32>>,
        value: Vec<Vec<f32>>,
    ) -> Result<AttentionOutput, TernVectorError> {
    ...
    fn compute_internal_static(
        ...
        if query.is_empty() || key.is_empty() || value.is_empty() {
            return Err(TernVectorError::InvalidInput("Empty input".to_string()));
        }
        ...
        if q_dim != k_dim {
            return Err(TernVectorError::DimensionMismatch {
                expected: q_dim,
                actual: k_dim,
            });
        }
    ...
    fn vec_to_array2(vec: &[Vec<f32>]) -> Result<Array2<f32>, TernVectorError> {
        if vec.is_empty() {
            return Err(TernVectorError::InvalidInput("Empty vector".to_string()));
        }
        ...
        Array2::from_shape_vec((rows, cols), flat)
            .map_err(|e| TernVectorError::ComputationError(e.to_string()))
    }
...
use napi::bindgen_prelude::*;
use napi_derive::napi;
use crate::{AttentionConfig, AttentionOutput, GraphStructure, TernVectorError};
...
impl HierarchicalAttention {
    ...
    fn compute_internal_static(
        ...
        if hierarchical_features.is_empty() {
            return Err(TernVectorError::InvalidInput("Empty hierarchy".to_string()));
        }
...
```json
{
  "name": "@ruvector/node",
  "version": "0.1.0",
  "description": "High-performance Node.js bindings for TernVector GNN latent space attention mechanisms",
  "main": "index.js",
  "types": "index.d.ts",
  "keywords": [
    "rust",
...
High-performance Node.js bindings for TernVector's GNN latent space attention mechanisms.
