...
TernVector's workspace contains 85+ crates. The following are directly relevant to the WASM cognitive stack:
...
**Status**: Proposed
**Context**: TernVector's WASM-compiled cognitive primitives exist independently without a unified execution model.
...
- [ ] npm publish for `-wasm` and `-node` variants (`npm whoami` = `rfi-irfos`)
...
The TernVector WASM cognitive stack integration is a 16-week effort that:
...
2. TernVector Workspace Cargo.toml (85+ crate definitions)
