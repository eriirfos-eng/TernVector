...
This document presents the architecture for a **verifiable WASM cognitive container** — a sealed, deterministic microkernel that composes TernVector's existing WASM-compiled crates (`cognitum-gate-kernel`, `ruvector-solver-wasm`, `ruvector-mincut-wasm`, `ruvector-gnn-wasm`) into a single execution unit with canonical witness chains, epoch-bounded computation, and Ed25519-verified integrity. The design leverages the existing kernel-pack system in `ruvector-wasm` (ADR-005) as the foundational infrastructure.
...
### 1.3 TernVector's Existing WASM Surface

TernVector already has the pieces:
