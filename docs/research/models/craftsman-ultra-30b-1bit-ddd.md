...
| **Auto-Labeling** | Classification of evaluation prompts as `resolved` (evidence redundancy > 3), `contested` (cluster disagreement > 0.4), or `indeterminate` (mincut fragility > 0.7) using TernVector retrieval signals, without manual annotation. |
...
| 33 | Merge weight learning strategy? | Embedding quality (AD-24) | Open | Fixed weights (w0=0.6, w1=0.3, w2=0.1) vs grid search vs small regression on eval set. Grid search is simple but doesn't generalize across domains. Regression requires labeled retrieval pairs. Can we use TernVector's own retrieval accuracy as the training signal? |
