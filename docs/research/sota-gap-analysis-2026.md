# TernVector SOTA Gap Analysis - March 2026

## Context

TernVector is a 187K LOC Rust codebase with 114 crates, 50+ npm packages, 43+ examples, 127 ADRs, and 211 research documents across 24 research tracks. This analysis identifies what's **missing** relative to 2024-2026 SOTA research from Google, Meta, DeepSeek, Microsoft, and the broader ML/systems community.
...
- **What's missing**: TernVector's HNSW is memory-resident. No SSD-backed ANN index exists.
- **Why it matters**: Microsoft's DiskANN (Vamana graph) enables billion-scale search on commodity SSDs with <5ms latency. Milvus, Qdrant, and LanceDB all support disk-backed indices. Without this, TernVector can't compete at >100M vector scale.
...
- **Why it matters**: PQ reduces memory 4-32x while maintaining >95% recall. Google's ScaNN anisotropic quantization and AQLM (Additive Quantization with codebooks) are current SOTA. Without PQ, TernVector can't efficiently serve 100M+ vectors.
...
- **Why it matters**: Production vector DBs need LSM-tree-style compaction for write-heavy workloads (Fresh-DiskANN, LanceDB append-optimized). TernVector's append-only RVF format is a good foundation but lacks index-level compaction.
...
- **Why it matters**: Self-supervised graph transformers eliminate need for labeled graph data. UniGraph enables cross-domain transfer. These would massively improve TernVector's GNN capabilities which currently require supervised training.
...
- **Why it matters**: TernVector has graph DB + vector search but doesn't combine them into structured RAG pipelines. Graph RAG achieves 30-60% better answers on complex queries vs naive RAG.
...
## 3. STRENGTHS (Where TernVector Leads or Matches SOTA)

| Capability | TernVector Status | SOTA Comparison |
|---|---|---|
...
**TernVector's unique strengths** are in mathematical foundations (mincut, spectral sparsification, sheaf coherence, hyperbolic geometry) and edge deployment (WASM, RVF format). These are genuine differentiators no competitor has.
...
**The strategic opportunity** is combining TernVector's unique graph/coherence capabilities with modern RAG techniques (Graph RAG, structured retrieval) to create a differentiated product that no pure vector DB can match.
...
1. **No JEPA (Joint Embedding Predictive Architecture)** — Meta's non-contrastive self-supervised learning is tracked nowhere in TernVector research docs despite being a paradigm shift from reconstruction-based methods
...
3. **No DPO/ORPO/KTO alignment** — TernVector has RLHF-adjacent concepts in SONA but no direct preference optimization methods
4. **No structured pruning** — SparseGPT and Wanda enable 50-60% weight removal with minimal quality loss; relevant for edge deployment which is a TernVector strength
...
- **Implication**: TernVector must implement SSD-backed search or risk irrelevance at scale
...
- **Implication**: TernVector's attention crate needs at minimum MLA + TurboQuant
