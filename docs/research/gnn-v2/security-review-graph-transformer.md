# Security Review: TernVector Graph Transformer Foundation Crates
...
This security review covers the four foundational crates that underpin the TernVector Graph Transformer: the formal verification engine (`ruvector-verified`), its WASM bindings (`ruvector-verified-wasm`), the GNN training pipeline (`ruvector-gnn`), and the attention mechanisms (`ruvector-attention`).
