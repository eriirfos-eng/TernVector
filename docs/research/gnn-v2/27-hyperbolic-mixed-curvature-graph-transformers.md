...
### Connection to TernVector

TernVector already has substantial infrastructure for this research direction:
...
This document extends TernVector's existing mixed-curvature capabilities toward full product manifold Graph Transformers with learned curvature fields.
...
TernVector's `LorentzCascadeAttention` implements this with multi-curvature heads operating at logarithmically-spaced curvatures, capturing hierarchy at multiple scales simultaneously.
...
where $\oplus_c$ is Mobius addition and $\otimes_c$ is Mobius scalar multiplication. TernVector's `poincare.rs` provides `mobius_add` and `mobius_scalar_mult` with full numerical stability.

The practical limitation is that Mobius aggregation is sequential -- each addition depends on the previous result. The Frechet mean (`frechet_mean` in TernVector) offers a parallel alternative via Riemannian gradient descent in the tangent space.
...
TernVector's `FusedCurvatureConfig` already defines this decomposition:
...
where the hyperbolic component uses tangent-space dot products (10-100x faster than geodesic distance per TernVector's `TangentSpaceMapper`) and the spherical component uses normalized inner products on the unit sphere.
...
This is the key extension beyond TernVector's current `MixedCurvatureConfig` (which uses a single curvature for the hyperbolic component). The research direction is to make $\kappa_i$ **learnable per-component**, enabling the model to discover which curvature best fits each subspace of the embedding.
...
where $\delta$ is the Gromov hyperbolicity (measuring how tree-like the graph is) and $\text{diam}(G)$ is the graph diameter. TernVector's solver crate provides the graph traversal primitives needed to compute both quantities sublinearly.
...
TernVector's `ruvector-attention/src/training/optimizer.rs` provides the foundation; extending it to Riemannian Adam requires adding `exp_map` and `log_map` calls (already available in `poincare.rs` and `lorentz_cascade.rs::tangent`).
...
TernVector's tangent space approach (`TangentSpaceMapper`) offers a practical middle ground: map to tangent space at the origin, perform standard operations, then map back. This is exact for small perturbations and provides 10-100x speedup over full geodesic operations.
...
This connects to TernVector's sheaf attention module (`ruvector-attention/src/sheaf/`), where restriction maps between stalks play a role analogous to parallel transport between fibers in the Lie group setting.
...
**Arbitrary Riemannian Manifolds (2032-2034):** Graph Transformers operate on manifolds defined by their learned metric tensor $g_{ij}(x)$ rather than restricted to constant-curvature spaces. The exponential map, parallel transport, and geodesic attention are computed via neural ODE solvers. TernVector's PDE attention module (`ruvector-attention/src/pde_attention/`) provides the diffusion-based foundation.
...
// Fused attention (single kernel, as in TernVector's fused_attention.rs):
...
## Implementation Roadmap for TernVector
...
**Owner:** TernVector Architecture Team
