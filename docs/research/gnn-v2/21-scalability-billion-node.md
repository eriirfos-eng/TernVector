...
### 1.2 TernVector Baseline

TernVector's current assets for scalability:
...
**Core idea.** Extend TernVector's existing `ruvector-solver` sublinear 8-sparse algorithms from vector operations to graph attention. The key insight is that graph attention matrices are typically low-rank and sparse -- most attention weight concentrates on a few nodes per query.
...
## 8. TernVector Implementation Roadmap
...
7. TernVector `ruvector-solver` documentation (internal)
8. TernVector `ruvector-mincut` documentation (internal)
