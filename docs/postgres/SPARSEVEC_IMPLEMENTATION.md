...
**`sparsevec_vector_l2_distance(sparse: SparseVec, dense: TernVector) -> f32`**
- L2 distance between sparse and dense vectors
- Uses scatter-gather for efficiency
- Handles mixed sparsity levels

**`sparsevec_vector_ip_distance(sparse: SparseVec, dense: TernVector) -> f32`**
- Inner product distance (sparse-dense)
- Scatter-gather optimization

**`sparsevec_vector_cosine_distance(sparse: SparseVec, dense: TernVector) -> f32`**
- Cosine distance (sparse-dense)

### 4. Conversion Functions

#### `sparsevec_to_vector(sparse: SparseVec) -> TernVector`
...
#### `vector_to_sparsevec(vector: TernVector, threshold: f32 = 0.0) -> SparseVec`
...
1. **Type System:** Uses same `SqlTranslatable` traits as `TernVector`
