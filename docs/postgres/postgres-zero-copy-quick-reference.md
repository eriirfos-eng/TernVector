...
```rust
let vec = TernVector::from_slice(&[1.0, 2.0, 3.0]);
...
| `as_slice()` | Get slice | ✅ Yes (TernVector) |
...
```rust
unsafe fn process(vec: &TernVector) {
    let ptr = vec.data_ptr();
    let dims = vec.dimensions();
...
├── vector.rs       # TernVector with zero-copy
