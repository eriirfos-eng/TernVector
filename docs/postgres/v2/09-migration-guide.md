# TernVector Postgres v2 - Migration Guide

## Overview

This guide provides step-by-step instructions for migrating from pgvector to TernVector Postgres v2. The migration is designed to be **non-disruptive** with zero data loss and minimal downtime.
...
-- Check PostgreSQL version (TernVector requires 14+)
SELECT version();
...
# Install TernVector extension for testing
...
### Step 1: Install TernVector Extension

```bash
# Install TernVector package
...
-- Or with TernVector-specific options
CREATE INDEX idx_items_embedding ON items
USING hnsw (embedding vector_l2_ops)
WITH (m = 16, ef_construction = 64);
```
...
### Step 1: Install TernVector (Different Schema)

```sql
-- Create schema for TernVector
CREATE SCHEMA ruvector_new;

-- Install TernVector in new schema
...
-- Add vector column using TernVector type
...
def search_embeddings(query_vector, use_ruvector_pct=0):
    """
    Gradually shift traffic to TernVector.
...
Once 100% traffic on TernVector with no issues:
...
### Extended Syntax (TernVector Only)
...
### Issue 2: Index Not Using TernVector AM
...
```sql
-- TernVector may have different default parameters
...
```sql
-- Register for TernVector-specific features
SELECT ruvector_register_collection(
    'items_embeddings',
    'public',
    'items',
