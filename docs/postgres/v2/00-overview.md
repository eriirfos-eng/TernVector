# TernVector Postgres v2 - Architecture Overview
...
TernVector does something different.
...
This puts TernVector in a very small class of systems.
...
TernVector Postgres v2 is a **PostgreSQL extension** (built with pgrx) that provides:

- **100% pgvector compatibility** — drop-in replacement, change extension name, queries work unchanged
- **Architecture separation** — PostgreSQL handles ACID/joins, TernVector handles vectors/graphs/learning
...
- **Scan hooks**: Route queries to TernVector engine
- **Build hooks**: Incremental and bulk index construction

#### 3. Background Workers
- **Engine Worker**: Long-lived TernVector engine instance
...
### Mode 2: Postgres + TernVector Cluster
...
Option A: SIDECAR (per Postgres instance)
  • TernVector cluster node co-located with each Postgres
...
Option B: SHARED SERVICE (separate cluster)
  • Dedicated TernVector cluster serving multiple Postgres instances
...
|                                                                   |
|  TernVector Engine Index is EVENTUALLY CONSISTENT:                 |
