# TernVector Postgres v2 - Phase 1: pgvector Compatibility

## Overview

Phase 1 establishes the foundation: a PostgreSQL extension that is **100% compatible** with pgvector. Any application using pgvector should be able to switch to TernVector by simply changing the extension name while keeping all queries unchanged.
...
pub extern "C" fn _PG_init() {
    ...
    pgrx::log!("TernVector {} initialized", VERSION);
}
...
impl TernVector {
    ...
    pub fn from_slice(data: &[f32]) -> *mut TernVector {
        let dim = data.len();
        if dim > 16000 {
            pgrx::error!("vector cannot have more than 16000 dimensions");
        }

        let size = size_of::<TernVector>() + dim * size_of::<f32>();

        unsafe {
            let ptr = pgrx::pg_sys::palloc0(size) as *mut TernVector;
...
fn vector_in(
    ...
    TernVector::from_slice(&values)
}

// Output function: vector -> '[1.0, 2.0, 3.0]'
#[pg_extern(immutable, parallel_safe)]
fn vector_out(vector: *mut TernVector) -> *mut std::ffi::c_char {
...
mod tests {
    ...
    fn test_vector_creation() {
        ...
        let vec = TernVector::from_slice(&data);
