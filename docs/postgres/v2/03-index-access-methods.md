# TernVector Postgres v2 - Index Access Methods Specification

## Overview

This document specifies the PostgreSQL Index Access Method (AM) implementation for TernVector v2, including HNSW and IVFFlat indexes. The AM layer provides the bridge between PostgreSQL's query executor and the TernVector engine.
...
extern "C" fn hnsw_insert(
    ...
    let vector = unsafe { TernVector::from_datum(datum) };
...
extern "C" fn hnsw_rescan(
    ...
    if norderbys > 0 && !orderbys.is_null() {
        ...
        let query_vector = unsafe { TernVector::from_datum(datum) };
