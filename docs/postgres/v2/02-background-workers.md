# TernVector Postgres v2 - Background Workers Specification

## Overview

TernVector v2 uses PostgreSQL background workers to maintain long-lived engine instances, perform maintenance operations, and run continuous learning pipelines. This document specifies the architecture, responsibilities, and communication patterns for each worker type.
...
### Purpose
The Engine Worker is the core TernVector instance that handles all vector operations. It maintains in-memory indexes and processes queries/mutations submitted via shared memory.
...
pub extern "C" fn ruvector_engine_worker_main(_arg: pg_sys::Datum) {
    pgrx::log!("TernVector engine worker starting");

    // Initialize shared memory
    let shmem = SharedMemory::attach().expect("Failed to attach shared memory");

    // Initialize engine
    let mut engine = TernVectorEngine::new(EngineWorkerConfig::default());
    ...
    pgrx::log!("TernVector engine worker stopped");
}
```
...
pub extern "C" fn ruvector_maintenance_worker_main(_arg: pg_sys::Datum) {
    pgrx::log!("TernVector maintenance worker starting");
    ...
    pgrx::log!("TernVector maintenance worker stopped");
}
```
...
pub extern "C" fn ruvector_integrity_worker_main(_arg: pg_sys::Datum) {
    pgrx::log!("TernVector integrity worker starting");
    ...
    pgrx::log!("TernVector integrity worker stopped");
}
```
