# Zero-Copy Distance Operators for TernVector PostgreSQL Extension

## Overview

This document describes the new zero-copy distance functions and SQL operators for the TernVector PostgreSQL extension. These functions provide significant performance improvements over the legacy array-based functions by:

1. **Zero-copy access**: Operating directly on TernVector types without memory allocation
...
1. **TernVector reception**: PostgreSQL passes the varlena datum directly
...
```sql
-- TernVector-based (faster, zero-copy)
