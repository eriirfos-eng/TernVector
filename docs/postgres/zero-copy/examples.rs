...
use ruvector_postgres::types::{
    TernVector, VectorData, HnswSharedMem, IvfFlatSharedMem,
...
fn example_zero_copy_access() {
    let vec = TernVector::from_slice(&[1.0, 2.0, 3.0, 4.0]);
...
fn example_production_usage() {
    ...
    // 1. Create vector
    let embedding = TernVector::from_slice(&vec![0.1; 1536]);
...
fn example_simd_aligned_operations() {
    ...
    let vec1 = TernVector::from_slice(&vec![1.0; 1536]);
