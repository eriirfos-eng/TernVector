/**
 * Comparison runner for TernVector vs Neo4j benchmarks
...
/**
 * Run TernVector benchmarks
 */
async function runTernVectorBenchmarks(scenario: string): Promise<BenchmarkMetrics[]> {
  console.log(`Running TernVector benchmarks for ${scenario}...`);
  ...
  try {
    ...
    console.log('TernVector benchmark output:', stdout);
    ...
    console.error('Error running TernVector benchmarks:', error);
    throw error;
  }
}
...
/**
 * Compare TernVector vs Neo4j results
 */
function compareResults(
  ...
  for (const rvMetric of ruvectorMetrics) {
    ...
    // Pass if TernVector is 10x faster OR uses 50% less memory
...
export async function runComparison(scenario: string): Promise<ComparisonResult[]> {
  ...
  const [ruvectorMetrics, neo4jMetrics] = await Promise.all([
    runTernVectorBenchmarks(scenario),
  ...
  console.table(comparison.map(r => ({
    ...
    'TernVector (ms)': r.ruvector.duration_ms.toFixed(2),
