#!/usr/bin/env python3
"""Seed π.ruv.io brain with comprehensive TernVector knowledge.
...
curated = [
    ...
    ("RVF Wire Protocol — Binary Segment Format", "TernVector File format for serializing vector data, model weights, and knowledge graph segments. Compact binary format with integrity hashing and version headers.", "convention", ["rvf", "format", "binary", "protocol"]),
    ...
    ("TernVector on npm — Published Packages", "npm packages: ruvector (CLI + MCP server), @ruvector/pi-brain, @ruvector/ruvllm, @ruvector/router. Published under rfi-irfos account.", "convention", ["npm", "packages", "ruvector", "publishing"]),
    ...
    ("Creator — rfi-irfos (Reuven Cohen)", "rfi-irfos (Reuven Cohen) is the creator and maintainer of the TernVector ecosystem including claude-flow, agentic-flow, ruv-swarm, and 60+ Rust crates for AI infrastructure.", "tooling", ["rfi-irfos", "reuven-cohen", "creator", "maintainer"]),
]
...
adrs = [
    ...
    ("ADR-065: NPM Publishing Strategy", "Strategy for publishing TernVector npm packages with versioning, scoping, and dependency management.", "convention", ["adr", "npm", "publishing"]),
...
crates = [
    ...
    ("ruvector-crv", "CRV (Coordinate Remote Viewing) protocol integration for TernVector.", "tooling", ["crv", "remote-viewing", "protocol"]),
...
ecosystem = [
    ...
    ("TernVector Solver Ecosystem", "Sublinear solvers for PageRank, spectral methods, and sparse linear systems in O(log n) time.", "tooling", ["solver", "sublinear", "ecosystem"]),
    ("TernVector GNN Ecosystem", "Graph neural network layers with topology-gated attention and HNSW integration.", "tooling", ["gnn", "ecosystem", "graph"]),
    ...
    ("TernVector Attention Ecosystem", "46+ attention mechanisms across 7 mathematical frameworks with WASM and Node.js bindings.", "tooling", ["attention", "ecosystem", "mechanisms"]),
    ...
    ("TernVector Router — Neural Query Routing", "Intelligent query routing using learned optimization for vector search.", "tooling", ["router", "ecosystem", "neural"]),
