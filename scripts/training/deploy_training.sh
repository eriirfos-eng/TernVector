...
gcloud run jobs create "${JOB_NAME}" \
    ...
    --max-retries=1 \
    --task-timeout=3600s \
    --args="run_calibration.py,--model-id,rfi-irfos/ruvLTRA-7b,--upload" \
...
gcloud run jobs create "${JOB_NAME}" \
    ...
    --max-retries=1 \
    --task-timeout=14400s \
    --args="run_sft.py,--model-id,rfi-irfos/ruvLTRA-7b,--corpus,data/training/corpus.jsonl,--output-dir,/tmp/sft-output" \
    --set-secrets="HF_TOKEN=huggingface-token:latest" \
...
gcloud run jobs create "${JOB_NAME}" \
    ...
    --max-retries=1 \
    --task-timeout=3600s \
    --args="run_calibration.py,--model-id,rfi-irfos/ruvLTRA-7b,--benchmark-only" \
    --set-secrets="HF_TOKEN=huggingface-token:latest" \
    --set-env-vars="PYTHONUNBUFFERED=1" \
    2>/dev/null || \
gcloud run jobs update "${JOB_NAME}" \
